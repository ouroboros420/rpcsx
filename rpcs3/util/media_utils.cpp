// NOTE: deliberately pinned to the FFmpeg 5.x API.
//
// Upstream v0.0.40 migrated this file to FFmpeg 7 (avcodec_get_supported_config
// with AVCodecConfig, and swr_alloc_set_opts2). The Android build links a
// prebuilt FFmpeg fixed at 5.1 (libavcodec 59) - see FFMPEG_VERSION in
// android/CMakeLists.txt - and RPCS3-Android/ffmpeg-android publishes no newer
// release, so those symbols simply do not exist here. Upstream's changes to this
// file are reverted on purpose; re-taking them will not compile for Android
// until a newer prebuilt FFmpeg is available.
#include "stdafx.h"
...
namespace utils
{
	...
	std::vector<ffmpeg_codec> list_ffmpeg_codecs(bool list_decoders)
	{
		...
		std::sort(codecs.begin(), codecs.end(), [](const ffmpeg_codec& l, const ffmpeg_codec& r)
			{
	...
	std::pair<bool, media_info> get_media_info(const std::string& path, s32 av_media_type)
	{
		...
		if (av_media_type == AVMEDIA_TYPE_UNKNOWN) // Let's use this for image info
		{
			...
			if (!success)
				media_log.error("get_media_info: failed to get image info for '%s'", path);
