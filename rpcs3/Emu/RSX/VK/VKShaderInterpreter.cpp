...
namespace vk
{
	...
	VKVertexProgram* shader_interpreter::build_vs(u64 compiler_options)
	{
		...
		builder << "\n"
				...
				<< vertex_instruction_start << ") readonly restrict buffer VertexInstructionBlock\n"
												 "{\n"
												 "	uint base_address;\n"
												 "	uint entry;\n"
												 "	uint output_mask;\n"
	...
	VKFragmentProgram* shader_interpreter::build_fs(u64 compiler_options)
	{
		...
		builder << "layout(std430, set=1, binding=" << fragment_instruction_start << ") readonly restrict buffer FragmentInstructionBlock\n"
																				"{\n"
																				"	uint shader_control;\n"
		...
		u32 fp_ctrl)
	{
		...
		if (vp_metadata.referenced_textures_mask)
			key.compiler_opt |= program_common::interpreter::COMPILER_OPT_ENABLE_VTX_TEXTURES;
