...
void VKVertexDecompilerThread::insertHeader(std::stringstream& OS)
{
	...
	// Only claim the extension where the device actually has it; requiring it on
	// a driver that lacks it (Adreno) fails every pipeline. The uniform-block
	// arrays below switch to concrete bounds instead - see vk::ubo_array_dim().
