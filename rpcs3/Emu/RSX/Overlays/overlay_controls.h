...
namespace rsx
{
	namespace overlays
	{
		...
		struct overlay_element
		{
			...
			u16 compute_vertically_centered(const overlay_element* other)
			{
				return compute_vertically_centered(other->h);
			}
			u16 compute_horizontally_centered(const overlay_element* other)
			{
				return compute_horizontally_centered(other->w);
			}
