...
static error_code query_memory_avc_mvc(ppu_thread& ppu, u32 profile_level, vm::ptr<AvcDecAttr> attr, const CellVdecAvcSpecificInfo* avc_specific_info)
{
	...
	if (avc_specific_info)
	{
		...
		{
			avc_params->max_decoded_frame_width = rx::aligned_div<u16>(avc_specific_info->maxDecodedFrameWidth, 0x10);
			avc_params->max_decoded_frame_height = rx::aligned_div<u16>(avc_specific_info->maxDecodedFrameHeight, 0x10);
		}
	}
