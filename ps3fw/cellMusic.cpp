	...
	sysutil_register_cb([context = *context, &music](ppu_thread& ppu) -> s32
		{
			...
			if (result)
				...
				cellMusic.todo("cellMusicSetSelectionContext2: failed. context = %s", music_selection_context::context_to_hex(context));
	...
	sysutil_register_cb([context = *context, &music](ppu_thread& ppu) -> s32
		{
			...
			if (result)
				cellMusic.success("cellMusicSetSelectionContext: new selection context = %s)", music.current_selection_context.to_string());
			else
				cellMusic.todo("cellMusicSetSelectionContext: failed. context = %s)", music_selection_context::context_to_hex(context));
