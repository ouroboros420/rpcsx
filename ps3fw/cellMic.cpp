	...
	const u8 channels)
{
	...
	const std::array<u32, 7> samplingrates = {
		raw_samplingrate, 48000u, 32000u, 24000u, 16000u, 12000u, 8000u};

	for (u32 samplingrate : samplingrates)
	{
		if (!std::any_of(samplingrates.cbegin() + 1, samplingrates.cend(),
				[samplingrate](u32 r)
				{
		...
		cellMic.notice("Trying sampling rate %d with %d channel(s)", samplingrate,
			num_channels);

		device =
			open_device(devices[0].name, samplingrate, num_al_channels, inbuf_size);
		if (!device)
		{
			continue;
		}
		...
		cellMic.notice("Using sampling rate %d and %d channel(s)", raw_samplingrate,
			num_channels);
		break;
	}
	...
	if (!device)
	{
		cellMic.error("Failed to open capture device '%s' (raw_samplingrate=%d, "
					  "num_al_channels=0x%x, inbuf_size=%d)",
			devices[0].name, raw_samplingrate, num_al_channels, inbuf_size);
#ifdef _WIN32
	...
	if (device_type == microphone_handler::singstar && devices.size() >= 2)
	{
		...
		num_al_channels = AL_FORMAT_MONO16;
		device = open_device(
			devices[1].name, raw_samplingrate, num_al_channels, inbuf_size);

		if (!device)
		{
			// Ignore it and move on
			cellMic.error("Failed to open 2nd SingStar capture device '%s' "
						  "(raw_samplingrate=%d, num_al_channels=0x%x, inbuf_size=%d)",
				devices[1].name, raw_samplingrate, num_al_channels, inbuf_size);
		}
		else
		{
