	...
	if (msg)
	{
		...
		for (u32 i = 0; i < msg->count && msg->npids; i++)
		{
			sceNp.trace("sceNpBasicSendMessageGui: NpId[%d] = %s", i,
				static_cast<char*>(&msg->npids[i].handle.data[0]));
		}
...
{
	sceNp.warning(
		"sceNpLookupCreateTitleCtx(communicationId=*0x%x(%s), selfNpId=0x%x)",
		communicationId, communicationId ? communicationId->data : "", selfNpId);
...
error_code sceNpSignalingGetLocalNetInfo(u32 ctx_id,
...
error_code sceNpUtilCmpNpId(vm::ptr<SceNpId> id1, vm::ptr<SceNpId> id2)
{
	sceNp.trace("sceNpUtilCmpNpId(id1=*0x%x(%s), id2=*0x%x(%s))", id1,
		id1 ? id1->handle.data : "", id2, id2 ? id2->handle.data : "");
