...
error_code
sceNpCommerceGetCurrencyInfo(vm::ptr<SceNpCommerceProductCategory> pc,
	vm::ptr<SceNpCommerceCurrencyInfo> info);
...
{
	sceNp.notice("commId: %s, msgId: %d, mainType: %d, subType: %d, subject: %s, "
				 "body: %s, data_size: %d",
		static_cast<const char*>(commId.data), msgId, mainType, subType,
		subject, body, data.size());
}
...
error_code npDrmIsAvailable(vm::cptr<u8> k_licensee_addr,
	vm::cptr<char> drm_path)
{
	if (!drm_path)
	{
	...
	if (k_licensee_addr)
	{
		...
		sceNp.notice("npDrmIsAvailable(): KLicense key or KLIC=%s",
			std::bit_cast<be_t<u128>>(k_licensee));
	}
	...
	ensure(vm::read_string(drm_path.addr(), 0x100, enc_drm_path, true),
	...
	const auto [fs_error, ppath, real_path, enc_file, type] =
		lv2_file::open(enc_drm_path, 0, 0);
	...
	if (magic == "SCE\0"_u32)
	{
		...
		if (verify_npdrm_self_headers(enc_file,
				reinterpret_cast<u8*>(&k_licensee)))
		{
			npdrmkeys.install_decryption_key(k_licensee);
		}
		else
		{
			sceNp.error(u8"npDrmIsAvailable(): Failed to verify sce file “%s”",
				enc_drm_path);
	...
	else if (magic == "NPD\0"_u32)
	{
		...
		if (VerifyEDATHeaderWithKLicense(enc_file, enc_drm_path,
				reinterpret_cast<u8*>(&k_licensee),
				&npd))
		{
			// Check if RAP-free
			...
			else
			{
				...
				if (fs::file rap_fd{rap_file})
				{
					...
					{
						sceNp.error(
							"npDrmIsAvailable(): Rap file too small: '%s' (%d bytes)",
							rap_file, rap_fd.size());
				...
				{
					sceNp.error("npDrmIsAvailable(): Rap file not found: '%s' (%s)",
						rap_file, fs::g_tls_error);
		...
		{
			sceNp.error(u8"npDrmIsAvailable(): Failed to verify npd file “%s”",
				enc_drm_path);
			return {SCE_NP_DRM_ERROR_NO_ENTITLEMENT, enc_drm_path};
		}
	}
	else
	{
		...
		sceNp.notice(
			u8"npDrmIsAvailable(): Assuming npdrm file is unencrypted at “%s”",
			enc_drm_path);
	}
...
error_code sceNpDrmIsAvailable(ppu_thread& ppu, vm::cptr<u8> k_licensee_addr,
	vm::cptr<char> drm_path)
{
	sceNp.warning("sceNpDrmIsAvailable(k_licensee=*0x%x, drm_path=*0x%x)",
		k_licensee_addr, drm_path);
...
error_code sceNpDrmIsAvailable2(ppu_thread& ppu, vm::cptr<u8> k_licensee_addr,
	vm::cptr<char> drm_path)
{
	sceNp.warning("sceNpDrmIsAvailable2(k_licensee=*0x%x, drm_path=*0x%x)",
		k_licensee_addr, drm_path);
...
error_code npDrmVerifyUpgradeLicense(vm::cptr<char> content_id)
{
	...
	ensure(vm::read_string(content_id.addr(), 0x2f, content_str, true),
		"Secret access violation");
...
error_code sceNpDrmExecuteGamePurchase()
{
	...
	// TODO:
	// 0. Check if the game can be purchased (return GAME_ERR_NOT_XMBBUY_CONTENT
	// otherwise)
	// 1. Send game termination request
	// 2. "Buy game" transaction (a.k.a. do nothing for now)
	// 3. Reboot game with CELL_GAME_ATTRIBUTE_XMBBUY attribute set
	// (cellGameBootCheck)
...
{
	sceNp.warning("sceNpDrmGetTimelimit(path=%s, time_remain=*0x%x)", path,
		time_remain);
	...
	std::string enc_drm_path;
	ensure(vm::read_string(path.addr(), 0x100, enc_drm_path, true),
		"Secret access violation");
	...
	// Read expiration time from NPD header which is Unix timestamp in
	// milliseconds
	if (magic == "SCE\0"_u32)
	{
		if (!get_npdrm_self_header(enc_file, npd))
		{
			sceNp.error("sceNpDrmGetTimelimit(): Failed to read NPD header from sce "
						"file '%s'",
				enc_drm_path);
			return {SCE_NP_DRM_ERROR_BAD_FORMAT, enc_drm_path};
		}
...
error_code sceNpDrmProcessExitSpawn(ppu_thread& ppu, vm::cptr<u8> klicensee,
	vm::cptr<char> path, vm::cpptr<char> argv,
	vm::cpptr<char> envp, u32 data,
	u32 data_size, s32 prio, u64 flags)
{
	sceNp.warning(
		"sceNpDrmProcessExitSpawn(klicensee=*0x%x, path=*0x%x, argv=**0x%x, "
		"envp=**0x%x, data=*0x%x, data_size=0x%x, prio=%d, flags=0x%x)",
		klicensee, path, argv, envp, data, data_size, prio, flags);
	...
	ppu_execute<&sys_game_process_exitspawn>(ppu, path, argv, envp, data,
		data_size, prio, flags);
	return CELL_OK;
}

error_code sceNpDrmProcessExitSpawn2(ppu_thread& ppu, vm::cptr<u8> klicensee,
	vm::cptr<char> path, vm::cpptr<char> argv,
	vm::cpptr<char> envp, u32 data,
	u32 data_size, s32 prio, u64 flags)
{
	sceNp.warning(
		"sceNpDrmProcessExitSpawn2(klicensee=*0x%x, path=*0x%x, argv=**0x%x, "
		"envp=**0x%x, data=*0x%x, data_size=0x%x, prio=%d, flags=0x%x)",
		klicensee, path, argv, envp, data, data_size, prio, flags);
	...
	ppu_execute<&sys_game_process_exitspawn2>(ppu, path, argv, envp, data,
		data_size, prio, flags);
	return CELL_OK;
}

error_code sceNpBasicRegisterHandler(vm::cptr<SceNpCommunicationId> context,
	vm::ptr<SceNpBasicEventHandler> handler,
	vm::ptr<void> arg)
{
	sceNp.warning(
		"sceNpBasicRegisterHandler(context=*0x%x(%s), handler=*0x%x, arg=*0x%x)",
		context, context ? context->data : "", handler, arg);
...
error_code sceNpBasicRegisterContextSensitiveHandler(
	vm::cptr<SceNpCommunicationId> context,
	vm::ptr<SceNpBasicEventHandler> handler, vm::ptr<void> arg)
{
	sceNp.notice("sceNpBasicRegisterContextSensitiveHandler(context=*0x%x(%s), "
				 "handler=*0x%x, arg=*0x%x)",
		context, context ? context->data : "", handler, arg);
...
error_code
sceNpBasicSetPresenceDetails(vm::cptr<SceNpBasicPresenceDetails> pres,
	u32 options)
{
	sceNp.warning("sceNpBasicSetPresenceDetails(pres=*0x%x, options=0x%x)", pres,
		options);
	...
	if (options < SCE_NP_BASIC_PRESENCE_OPTIONS_SET_DATA ||
		options > SCE_NP_BASIC_PRESENCE_OPTIONS_ALL_OPTIONS)
	{
		return SCE_NP_BASIC_ERROR_INVALID_ARGUMENT;
	}
	...
	{
		pr_status = std::optional(
			std::string(reinterpret_cast<const char*>(&pres->status[0])));
	}
...
error_code
sceNpBasicSetPresenceDetails2(vm::cptr<SceNpBasicPresenceDetails2> pres,
	u32 options)
{
	sceNp.warning("sceNpBasicSetPresenceDetails2(pres=*0x%x, options=0x%x)", pres,
		options);
	...
	if (options < SCE_NP_BASIC_PRESENCE_OPTIONS_SET_DATA ||
		options > SCE_NP_BASIC_PRESENCE_OPTIONS_ALL_OPTIONS ||
		pres->struct_size != sizeof(SceNpBasicPresenceDetails2))
	{
		return SCE_NP_BASIC_ERROR_INVALID_ARGUMENT;
	}
...
error_code _sceNpBasicSendMessage(vm::cptr<SceNpId> to, vm::cptr<void> data, u32 size, bool rate_limited)
{
	...
	message_data msg_data = {
		...
		.msgFeatures = {},
		.data = std::vector<u8>(static_cast<const u8*>(data.get_ptr()),
			static_cast<const u8*>(data.get_ptr()) + size)};
...
error_code sceNpBasicSendMessage(vm::cptr<SceNpId> to, vm::cptr<void> data,
	u32 size)
{
	sceNp.warning("sceNpBasicSendMessage(to=*0x%x, data=*0x%x, size=%d)", to,
		data, size);
	return _sceNpBasicSendMessage(to, data, size, false);
}

// This function is sceNpBasicSendMessage + a rate limiter that will return
// SCE_NP_BASIC_ERROR_BUSY if it too many messages have been sent
error_code sceNpBasicLimited_0xEB42E2E6(vm::cptr<SceNpId> to,
	vm::cptr<void> data, u32 size)
{
	sceNp.warning("sceNpBasicLimited_0xEB42E2E6(to=*0x%x, data=*0x%x, size=%d)",
		to, data, size);
...
{
	sceNp.warning("sceNpBasicSendMessageGui(msg=*0x%x, containerId=%d)", msg,
		containerId);

	if (msg)
	{
		sceNp.notice("sceNpBasicSendMessageGui: msgId: %d, mainType: %d, subType: "
					 "%d, msgFeatures: %d, count: %d, npids: *0x%x",
			msg->msgId, msg->mainType, msg->subType, msg->msgFeatures,
			msg->count, msg->npids);
		for (u32 i = 0; i < msg->count && msg->npids; i++)
		{
			sceNp.trace("sceNpBasicSendMessageGui: NpId[%d] = %s", i,
				np::npid_to_string(msg->npids[i]));
		}
	...
	if (!msg || msg->count > SCE_NP_BASIC_SEND_MESSAGE_MAX_RECIPIENTS ||
		(msg->msgFeatures & ~SCE_NP_BASIC_MESSAGE_FEATURES_ALL_FEATURES) ||
	...
	if (!(msg->msgFeatures & SCE_NP_BASIC_MESSAGE_FEATURES_BOOTABLE))
	{
		...
		case SCE_NP_BASIC_MESSAGE_MAIN_TYPE_URL_ATTACHMENT:
			if (msg->subType !=
				SCE_NP_BASIC_MESSAGE_URL_ATTACHMENT_SUBTYPE_ACTION_USE)
				return SCE_NP_BASIC_ERROR_NOT_SUPPORTED;
			break;
		case SCE_NP_BASIC_MESSAGE_MAIN_TYPE_GENERAL:
		case SCE_NP_BASIC_MESSAGE_MAIN_TYPE_ADD_FRIEND:
			if (msg->subType != 0) // SCE_NP_BASIC_MESSAGE_GENERAL_SUBTYPE_NONE,
			                       // SCE_NP_BASIC_MESSAGE_ADD_FRIEND_SUBTYPE_NONE
				return SCE_NP_BASIC_ERROR_NOT_SUPPORTED;
			if (msg->data)
				return SCE_NP_BASIC_ERROR_INVALID_ARGUMENT;
			break;
	...
	sceNp.trace("Message Data:\n%s",
		fmt::buf_to_hexstring(msg->data.get_ptr(), msg->size));
	...
	else
	{
		...
		Emu.BlockingCallFromMainThread([=, &result, msg_data = std::move(msg_data),
										   npids = std::move(npids)]() mutable
			{
...
error_code sceNpBasicSendMessageAttachment(ppu_thread& ppu,
	vm::cptr<SceNpId> to,
	vm::cptr<char> subject,
	vm::cptr<char> body,
	vm::cptr<void> data, u32 size,
	sys_memory_container_t containerId)
{
	sceNp.warning("sceNpBasicSendMessageAttachment(to=*0x%x, subject=%s, "
				  "body=%s, data=%s, size=%d, containerId=%d)",
		to, subject, body, data, size, containerId);
...
error_code sceNpBasicRecvMessageAttachment(ppu_thread& ppu,
	sys_memory_container_t containerId)
{
...
error_code sceNpBasicRecvMessageAttachmentLoad(SceNpBasicAttachmentDataId id,
	vm::ptr<void> buffer,
	vm::ptr<u32> size)
{
	sceNp.warning(
		"sceNpBasicRecvMessageAttachmentLoad(id=%d, buffer=*0x%x, size=*0x%x)",
		id, buffer, size);
	...
	if (id != SCE_NP_BASIC_SELECTED_INVITATION_DATA &&
		id != SCE_NP_BASIC_SELECTED_MESSAGE_DATA)
	{
	...
	const u32 size_to_copy =
		std::min(static_cast<u32>(msg.data.size()), orig_size);
	memcpy(buffer.get_ptr(), msg.data.data(), size_to_copy);

	sceNp.trace(
		"Message Data received:\n%s",
		fmt::buf_to_hexstring(static_cast<u8*>(buffer.get_ptr()), size_to_copy));
...
error_code sceNpBasicRecvMessageCustom(ppu_thread& ppu, u16 mainType,
	u32 recvOptions,
	sys_memory_container_t containerId)
{
	sceNp.warning("sceNpBasicRecvMessageCustom(mainType=%d, recvOptions=%d, "
				  "containerId=%d)",
		mainType, recvOptions, containerId);
...
error_code recv_message_gui(ppu_thread& ppu, u16 mainType, u32 recvOptions)
{
	...
	if (auto manager = g_fxo->try_get<rsx::overlays::display_manager>())
	{
		...
		result =
			recv_dlg->Exec(static_cast<SceNpBasicMessageMainType>(mainType),
				static_cast<SceNpBasicMessageRecvOptions>(recvOptions),
				recv_result, chosen_msg_id);
	}
	else
	{
		input::SetIntercepted(true);

		Emu.BlockingCallFromMainThread([=, &result, &recv_result,
										   &chosen_msg_id]()
			{
	...
	{
		sceNp.fatal(
			"sceNpBasicRecvMessageCustom: message is invalid: chosen_msg_id=%d",
			chosen_msg_id);
	...
	if (mainType == SCE_NP_BASIC_MESSAGE_MAIN_TYPE_DATA_ATTACHMENT)
	{
		...
		SceNpBasicAttachmentData* att_data =
			reinterpret_cast<SceNpBasicAttachmentData*>(to_add.data.data());
		*att_data = data;
	...
	else
	{
		...
		SceNpBasicExtendedAttachmentData* att_data =
			reinterpret_cast<SceNpBasicExtendedAttachmentData*>(
				to_add.data.data());
		att_data->flags = 0; // ?
		att_data->msgId = chosen_msg_id;
		att_data->data = data;
		att_data->userAction = recv_result;
		att_data->markedAsUsed =
			(recvOptions & SCE_NP_BASIC_RECV_MESSAGE_OPTIONS_PRESERVE) ? 0 : 1;
	...
	// Is this sent if used from home menu but not from
	// sceNpBasicRecvMessageCustom, not sure
...
error_code sceNpBasicAddFriend(ppu_thread& ppu, vm::cptr<SceNpId> contact,
	vm::cptr<char> body,
	sys_memory_container_t containerId)
{
	sceNp.warning("sceNpBasicAddFriend(contact=*0x%x, body=%s, containerId=%d)",
		contact, body, containerId);
...
{
	sceNp.trace("sceNpBasicGetFriendListEntry(index=%d, npid=*0x%x)", index,
		npid);
...
error_code
sceNpBasicGetFriendPresenceByIndex(u32 index, vm::ptr<SceNpUserInfo> user,
	vm::ptr<SceNpBasicPresenceDetails> pres,
	u32 options)
{
	sceNp.warning("sceNpBasicGetFriendPresenceByIndex(index=%d, user=*0x%x, "
				  "pres=*0x%x, options=%d)",
		index, user, pres, options);
	...
	if (!user)
	{
		// TODO: check index and (options &
		// SCE_NP_BASIC_PRESENCE_OPTIONS_ALL_OPTIONS) depending on fw
		return SCE_NP_BASIC_ERROR_INVALID_ARGUMENT;
	}

	return nph.get_friend_presence_by_index(index, user.get_ptr(),
		pres.get_ptr());
}

error_code
sceNpBasicGetFriendPresenceByIndex2(u32 index, vm::ptr<SceNpUserInfo> user,
	vm::ptr<SceNpBasicPresenceDetails2> pres,
	u32 options)
{
	sceNp.warning("sceNpBasicGetFriendPresenceByIndex2(index=%d, user=*0x%x, "
				  "pres=*0x%x, options=%d)",
		index, user, pres, options);
...
error_code
sceNpBasicGetFriendPresenceByNpId(vm::cptr<SceNpId> npid,
	vm::ptr<SceNpBasicPresenceDetails> pres,
	u32 options)
{
	sceNp.warning(
		"sceNpBasicGetFriendPresenceByNpId(npid=*0x%x, pres=*0x%x, options=%d)",
		npid, pres, options);
	...
	if (!npid)
	{
		// TODO: check (options & SCE_NP_BASIC_PRESENCE_OPTIONS_ALL_OPTIONS)
		// depending on fw
		return SCE_NP_BASIC_ERROR_INVALID_ARGUMENT;
	}
...
error_code
sceNpBasicGetFriendPresenceByNpId2(vm::cptr<SceNpId> npid,
	vm::ptr<SceNpBasicPresenceDetails2> pres,
	u32 options)
{
	sceNp.warning(
		"sceNpBasicGetFriendPresenceByNpId2(npid=*0x%x, pres=*0x%x, options=%d)",
		npid, pres, options);
...
error_code sceNpBasicAddPlayersHistory(vm::cptr<SceNpId> npid,
	vm::cptr<char> description)
{
	sceNp.warning("sceNpBasicAddPlayersHistory(npid=*0x%x, description=*0x%x)",
		npid, description);
	...
	if (description &&
		strlen(description.get_ptr()) > SCE_NP_BASIC_DESCRIPTION_CHARACTER_MAX)
	{
		return SCE_NP_BASIC_ERROR_EXCEEDS_MAX;
	}

	nph.add_player_to_history(npid.get_ptr(),
		description ? description.get_ptr() : nullptr);

	return CELL_OK;
}

error_code sceNpBasicAddPlayersHistoryAsync(vm::cptr<SceNpId> npids, u32 count,
	vm::cptr<char> description,
	vm::ptr<u32> reqId)
{
	sceNp.warning("sceNpBasicAddPlayersHistoryAsync(npids=*0x%x, count=%d, "
				  "description=*0x%x, reqId=*0x%x)",
		npids, count, description, reqId);
	...
	auto req_id = nph.add_players_to_history(
		npids.get_ptr(), description ? description.get_ptr() : nullptr, count);
...
{
	sceNp.warning(
		"sceNpBasicGetPlayersHistoryEntryCount(options=%d, count=*0x%x)", options,
		count);
...
error_code sceNpBasicGetPlayersHistoryEntry(u32 options, u32 index,
	vm::ptr<SceNpId> npid)
{
	sceNp.warning(
		"sceNpBasicGetPlayersHistoryEntry(options=%d, index=%d, npid=*0x%x)",
		options, index, npid);
...
error_code sceNpBasicGetMessageAttachmentEntry(u32 index,
	vm::ptr<SceNpUserInfo> from)
{
	sceNp.todo("sceNpBasicGetMessageAttachmentEntry(index=%d, from=*0x%x)", index,
		from);
...
error_code sceNpBasicGetCustomInvitationEntry(u32 index,
	vm::ptr<SceNpUserInfo> from)
{
	sceNp.todo("sceNpBasicGetCustomInvitationEntry(index=%d, from=*0x%x)", index,
		from);
...
error_code sceNpBasicGetMatchingInvitationEntry(u32 index,
	vm::ptr<SceNpUserInfo> from)
{
	sceNp.todo("sceNpBasicGetMatchingInvitationEntry(index=%d, from=*0x%x)",
		index, from);
...
error_code sceNpBasicGetClanMessageEntry(u32 index,
	vm::ptr<SceNpUserInfo> from)
{
	sceNp.todo("sceNpBasicGetClanMessageEntry(index=%d, from=*0x%x)", index,
		from);
...
{
	sceNp.todo("sceNpBasicGetMessageEntryCount(type=%d, count=*0x%x)", type,
		count);
...
error_code sceNpBasicGetMessageEntry(u32 type, u32 index,
	vm::ptr<SceNpUserInfo> from)
{
	sceNp.todo("sceNpBasicGetMessageEntry(type=%d, index=%d, from=*0x%x)", type,
		index, from);
...
error_code sceNpBasicGetEvent(vm::ptr<s32> event, vm::ptr<SceNpUserInfo> from,
	vm::ptr<u8> data, vm::ptr<u32> size)
{
	sceNp.trace(
		"sceNpBasicGetEvent(event=*0x%x, from=*0x%x, data=*0x%x, size=*0x%x)",
		event, from, data, size);
...
error_code sceNpCommerceCreateCtx(u32 version, vm::ptr<SceNpId> npId,
	vm::ptr<SceNpCommerceHandler> handler,
	vm::ptr<void> arg, vm::ptr<u32> ctx_id)
{
	sceNp.todo("sceNpCommerceCreateCtx(version=%d, event=*0x%x, from=*0x%x, "
			   "arg=*0x%x, ctx_id=*0x%x)",
		version, npId, handler, arg, ctx_id);
...
error_code
sceNpCommerceInitProductCategory(vm::ptr<SceNpCommerceProductCategory> pc,
	vm::cptr<void> data, u32 data_size)
{
	sceNp.todo(
		"sceNpCommerceInitProductCategory(pc=*0x%x, data=*0x%x, data_size=%d)",
		pc, data, data_size);
...
void sceNpCommerceDestroyProductCategory(
...
error_code sceNpCommerceGetProductCategoryStart(u32 ctx_id,
	vm::cptr<char> category_id,
	s32 lang_code,
	vm::ptr<u32> req_id)
{
	sceNp.todo("sceNpCommerceGetProductCategoryStart(ctx_id=%d, category_id=%s, "
			   "lang_code=%d, req_id=*0x%x)",
		ctx_id, category_id, lang_code, req_id);
...
error_code sceNpCommerceGetProductCategoryResult(u32 req_id, vm::ptr<void> buf,
	u32 buf_size,
	vm::ptr<u32> fill_size)
{
	sceNp.todo("sceNpCommerceGetProductCategoryResult(req_id=%d, buf=*0x%x, "
			   "buf_size=%d, fill_size=*0x%x)",
		req_id, buf, buf_size, fill_size);
...
vm::cptr<char>
sceNpCommerceGetProductId(vm::ptr<SceNpCommerceProductSkuInfo> info)
{
...
vm::cptr<char>
sceNpCommerceGetProductName(vm::ptr<SceNpCommerceProductSkuInfo> info)
{
...
vm::cptr<char>
sceNpCommerceGetCategoryDescription(vm::ptr<SceNpCommerceCategoryInfo> info)
{
...
vm::cptr<char>
sceNpCommerceGetCategoryId(vm::ptr<SceNpCommerceCategoryInfo> info)
{
...
vm::cptr<char>
sceNpCommerceGetCategoryImageURL(vm::ptr<SceNpCommerceCategoryInfo> info)
{
...
error_code
sceNpCommerceGetCategoryInfo(vm::ptr<SceNpCommerceProductCategory> pc,
	vm::ptr<SceNpCommerceCategoryInfo> info)
{
...
vm::cptr<char>
sceNpCommerceGetCategoryName(vm::ptr<SceNpCommerceCategoryInfo> info)
{
...
vm::cptr<char>
sceNpCommerceGetCurrencyCode(vm::ptr<SceNpCommerceCurrencyInfo> info)
{
...
error_code
sceNpCommerceGetNumOfChildCategory(vm::ptr<SceNpCommerceProductCategory> pc,
	vm::ptr<u32> num)
{
	sceNp.todo("sceNpCommerceGetNumOfChildCategory(pc=*0x%x, num=*0x%x)", pc,
		num);
...
error_code
sceNpCommerceGetNumOfChildProductSku(vm::ptr<SceNpCommerceProductCategory> pc,
	vm::ptr<u32> num)
{
	sceNp.todo("sceNpCommerceGetNumOfChildProductSku(pc=*0x%x, num=*0x%x)", pc,
		num);
...
vm::cptr<char>
sceNpCommerceGetSkuDescription(vm::ptr<SceNpCommerceProductSkuInfo> info)
{
...
vm::cptr<char>
sceNpCommerceGetSkuId(vm::ptr<SceNpCommerceProductSkuInfo> info)
{
...
vm::cptr<char>
sceNpCommerceGetSkuImageURL(vm::ptr<SceNpCommerceProductSkuInfo> info)
{
...
vm::cptr<char>
sceNpCommerceGetSkuName(vm::ptr<SceNpCommerceProductSkuInfo> info)
{
...
void sceNpCommerceGetSkuPrice(vm::ptr<SceNpCommerceProductSkuInfo> info,
	vm::ptr<SceNpCommercePrice> price)
{
	...
	if (info && info->pc && info->pc->dval)
	{
		...
		// price->fractional = something - static_cast<int>(price->integer) *
		// info->pc->dval;
		return;
	}
...
vm::cptr<char>
sceNpCommerceGetSkuUserData(vm::ptr<SceNpCommerceProductSkuInfo> info)
{
...
error_code sceNpCommerceSetDataFlagStart(u32 arg1, u32 arg2, u32 arg3, u32 arg4,
	u32 arg5)
{
	sceNp.todo("sceNpCommerceSetDataFlagStart(arg1=0x%x, arg2=0x%x, arg3=0x%x, "
			   "arg4=0x%x, arg5=0x%x)",
...
error_code sceNpCommerceGetDataFlagStart(u32 arg1, u32 arg2, u32 arg3, u32 arg4,
	u32 arg5)
{
	sceNp.todo("sceNpCommerceGetDataFlagStart(arg1=0x%x, arg2=0x%x, arg3=0x%x, "
			   "arg4=0x%x, arg5=0x%x)",
		arg1, arg2, arg3, arg4, arg5);
...
{
	sceNp.todo("sceNpCommerceGetDataFlagState(arg1=0x%x, arg2=0x%x, arg3=0x%x)",
		arg1, arg2, arg3);
...
error_code
sceNpCommerceGetChildCategoryInfo(vm::ptr<SceNpCommerceProductCategory> pc,
	u32 child_index,
	vm::ptr<SceNpCommerceCategoryInfo> info)
{
	sceNp.todo(
		"sceNpCommerceGetChildCategoryInfo(pc=*0x%x, child_index=%d, info=*0x%x)",
		pc, child_index, info);
...
error_code
sceNpCommerceGetChildProductSkuInfo(vm::ptr<SceNpCommerceProductCategory> pc,
	u32 child_index,
	vm::ptr<SceNpCommerceProductSkuInfo> info)
{
	sceNp.todo("sceNpCommerceGetChildProductSkuInfo(pc=*0x%x, child_index=%d, "
			   "info=*0x%x)",
		pc, child_index, info);
...
error_code sceNpCommerceDoCheckoutStartAsync(u32 ctx_id,
	vm::cpptr<char> sku_ids,
	u32 sku_num,
	sys_memory_container_t container,
	vm::ptr<u32> req_id)
{
	sceNp.todo("sceNpCommerceDoCheckoutStartAsync(ctx_id=%d, sku_ids=*0x%x, "
			   "sku_num=%d, container=%d, req_id=*0x%x)",
		ctx_id, sku_ids, sku_num, container, req_id);
...
error_code
sceNpCustomMenuRegisterActions(vm::cptr<SceNpCustomMenu> menu,
	vm::ptr<SceNpCustomMenuEventHandler> handler,
	vm::ptr<void> userArg, u64 options)
{
	sceNp.todo("sceNpCustomMenuRegisterActions(menu=*0x%x, handler=*0x%x, "
			   "userArg=*0x%x, options=0x%x)",
		menu, handler, userArg, options);
	...
	for (u32 i = 0; i < menu->numActions; i++)
	{
		...
		if (!memchr(menu->actions[i].name.get_ptr(), '\0',
				SCE_NP_CUSTOM_MENU_ACTION_CHARACTER_MAX))
		{
			return SCE_NP_CUSTOM_MENU_ERROR_EXCEEDS_MAX;
		}
		...
		sceNp.notice("Registering menu action: id=%d, mask=0x%x, name=%s",
			action.id, action.mask, action.name);
...
error_code
sceNpCustomMenuActionSetActivation(vm::cptr<SceNpCustomMenuIndexArray> array,
	u64 options)
{
	sceNp.todo("sceNpCustomMenuActionSetActivation(array=*0x%x, options=0x%x)",
		array, options);
...
error_code sceNpCustomMenuRegisterExceptionList(
	vm::cptr<SceNpCustomMenuActionExceptions> items, u32 numItems,
	u64 options)
{
	sceNp.todo("sceNpCustomMenuRegisterExceptionList(items=*0x%x, numItems=%d, "
			   "options=0x%x)",
		items, numItems, options);
...
error_code sceNpFriendlist(vm::ptr<SceNpFriendlistResultHandler> resultHandler,
	vm::ptr<void> userArg,
	sys_memory_container_t containerId)
{
	sceNp.warning(
		"sceNpFriendlist(resultHandler=*0x%x, userArg=*0x%x, containerId=%d)",
		resultHandler, userArg, containerId);
...
error_code
sceNpFriendlistCustom(SceNpFriendlistCustomOptions options,
	vm::ptr<SceNpFriendlistResultHandler> resultHandler,
	vm::ptr<void> userArg,
	sys_memory_container_t containerId)
{
	sceNp.warning("sceNpFriendlistCustom(options=0x%x, resultHandler=*0x%x, "
				  "userArg=*0x%x, containerId=%d)",
		options, resultHandler, userArg, containerId);
...
error_code
sceNpLookupCreateTitleCtx(vm::cptr<SceNpCommunicationId> communicationId,
	vm::cptr<SceNpId> selfNpId)
{
...
error_code sceNpLookupNpId(s32 transId, vm::cptr<SceNpOnlineId> onlineId,
	vm::ptr<SceNpId> npId, vm::ptr<void> option)
{
	sceNp.todo(
		"sceNpLookupNpId(transId=%d, onlineId=*0x%x, npId=*0x%x, option=*0x%x)",
		transId, onlineId, npId, option);
...
error_code sceNpLookupNpIdAsync(s32 transId, vm::cptr<SceNpOnlineId> onlineId,
	vm::ptr<SceNpId> npId, s32 prio,
	vm::ptr<void> option)
{
	sceNp.todo("sceNpLookupNpIdAsync(transId=%d, onlineId=*0x%x, npId=*0x%x, "
			   "prio=%d, option=*0x%x)",
		transId, onlineId, npId, prio, option);
...
error_code sceNpLookupUserProfile(s32 transId, vm::cptr<SceNpId> npId,
	vm::ptr<SceNpUserInfo> userInfo,
	vm::ptr<SceNpAboutMe> aboutMe,
	vm::ptr<SceNpMyLanguages> languages,
	vm::ptr<SceNpCountryCode> countryCode,
	vm::ptr<SceNpAvatarImage> avatarImage,
	vm::ptr<void> option)
{
	sceNp.todo("sceNpLookupUserProfile(transId=%d, npId=*0x%x, userInfo=*0x%x, "
			   "aboutMe=*0x%x, languages=*0x%x, countryCode=*0x%x, "
			   "avatarImage=*0x%x, option=*0x%x)",
		transId, npId, userInfo, aboutMe, languages, countryCode,
		avatarImage, option);
...
error_code sceNpLookupUserProfileAsync(s32 transId, vm::cptr<SceNpId> npId,
	vm::ptr<SceNpUserInfo> userInfo,
	vm::ptr<SceNpAboutMe> aboutMe,
	vm::ptr<SceNpMyLanguages> languages,
	vm::ptr<SceNpCountryCode> countryCode,
	vm::ptr<SceNpAvatarImage> avatarImage,
	s32 prio, vm::ptr<void> option)
{
	sceNp.todo("sceNpLookupUserProfileAsync(transId=%d, npId=*0x%x, "
			   "userInfo=*0x%x, aboutMe=*0x%x, languages=*0x%x, "
			   "countryCode=*0x%x, avatarImage=*0x%x, prio=%d, option=*0x%x)",
		transId, npId, userInfo, aboutMe, languages, countryCode,
		avatarImage, prio, option);
...
error_code sceNpLookupUserProfileWithAvatarSize(
	s32 transId, s32 avatarSizeType, vm::cptr<SceNpId> npId,
	vm::ptr<SceNpUserInfo> userInfo, vm::ptr<SceNpAboutMe> aboutMe,
	vm::ptr<SceNpMyLanguages> languages, vm::ptr<SceNpCountryCode> countryCode,
	vm::ptr<void> avatarImageData, u32 avatarImageDataMaxSize,
	vm::ptr<u32> avatarImageDataSize, vm::ptr<void> option)
{
	sceNp.todo(
		"sceNpLookupUserProfileWithAvatarSize(transId=%d, avatarSizeType=%d, "
		"npId=*0x%x, userInfo=*0x%x, aboutMe=*0x%x, languages=*0x%x, "
		"countryCode=*0x%x, avatarImageData=*0x%x, "
		"avatarImageDataMaxSize=%d, avatarImageDataSize=*0x%x, option=*0x%x)",
		transId, avatarSizeType, npId, userInfo, aboutMe, languages, countryCode,
		avatarImageData, avatarImageDataMaxSize, avatarImageDataSize, option);
...
error_code sceNpLookupUserProfileWithAvatarSizeAsync(
	s32 transId, s32 avatarSizeType, vm::cptr<SceNpId> npId,
	vm::ptr<SceNpUserInfo> userInfo, vm::ptr<SceNpAboutMe> aboutMe,
	vm::ptr<SceNpMyLanguages> languages, vm::ptr<SceNpCountryCode> countryCode,
	vm::ptr<void> avatarImageData, u32 avatarImageDataMaxSize,
	vm::ptr<u32> avatarImageDataSize, s32 prio, vm::ptr<void> option)
{
	sceNp.todo("sceNpLookupUserProfileWithAvatarSizeAsync(transId=%d, "
			   "avatarSizeType=%d, npId=*0x%x, userInfo=*0x%x, aboutMe=*0x%x, "
			   "languages=*0x%x, countryCode=*0x%x, avatarImageData=*0x%x, "
			   "avatarImageDataMaxSize=%d, avatarImageDataSize=*0x%x, prio=%d, "
			   "option=*0x%x)",
		transId, avatarSizeType, npId, userInfo, aboutMe, languages,
		countryCode, avatarImageData, avatarImageDataMaxSize,
		avatarImageDataSize, prio, option);
...
error_code sceNpLookupAvatarImage(s32 transId,
	vm::ptr<SceNpAvatarUrl> avatarUrl,
	vm::ptr<SceNpAvatarImage> avatarImage,
	vm::ptr<void> option)
{
	sceNp.todo("sceNpLookupAvatarImage(transId=%d, avatarUrl=*0x%x, "
			   "avatarImage=*0x%x, option=*0x%x)",
		transId, avatarUrl, avatarImage, option);
...
error_code sceNpLookupAvatarImageAsync(s32 transId,
	vm::ptr<SceNpAvatarUrl> avatarUrl,
	vm::ptr<SceNpAvatarImage> avatarImage,
	s32 prio, vm::ptr<void> option)
{
	sceNp.todo("sceNpLookupAvatarImageAsync(transId=%d, avatarUrl=*0x%x, "
			   "avatarImage=*0x%x, prio=%d, option=*0x%x)",
...
error_code sceNpLookupTitleSmallStorage(s32 transId, vm::ptr<void> data,
	u32 maxSize, vm::ptr<u32> contentLength,
	vm::ptr<void> option)
{
	sceNp.todo("sceNpLookupTitleSmallStorage(transId=%d, data=*0x%x, maxSize=%d, "
			   "contentLength=*0x%x, option=*0x%x)",
		transId, data, maxSize, contentLength, option);
...
error_code sceNpLookupTitleSmallStorageAsync(s32 transId, vm::ptr<void> data,
	u32 maxSize,
	vm::ptr<u32> contentLength,
	s32 prio, vm::ptr<void> option)
{
	sceNp.todo("sceNpLookupTitleSmallStorageAsync(transId=%d, data=*0x%x, "
			   "maxSize=%d, contentLength=*0x%x, prio=%d, option=*0x%x)",
		transId, data, maxSize, contentLength, prio, option);
	...
	// TSS are game specific data we don't have access to, set buf to 0, return
	// size 0
...
error_code sceNpManagerRegisterCallback(vm::ptr<SceNpManagerCallback> callback,
	vm::ptr<void> arg)
{
	sceNp.warning("sceNpManagerRegisterCallback(callback=*0x%x, arg=*0x%x)",
		callback, arg);
...
error_code sceNpManagerGetOnlineId(vm::ptr<SceNpOnlineId> onlineId)
{
	...
	if (nph.get_psn_status() != SCE_NP_MANAGER_STATUS_LOGGING_IN &&
		nph.get_psn_status() != SCE_NP_MANAGER_STATUS_ONLINE)
	{
		return SCE_NP_ERROR_INVALID_STATE;
	}
...
error_code sceNpManagerGetOnlineName(ppu_thread& ppu,
	vm::ptr<SceNpOnlineName> onlineName)
{
	ppu.state += cpu_flag::wait;
...
error_code sceNpManagerGetAvatarUrl(ppu_thread& ppu,
	vm::ptr<SceNpAvatarUrl> avatarUrl)
{
	ppu.state += cpu_flag::wait;
...
error_code sceNpManagerGetAccountRegion(vm::ptr<SceNpCountryCode> countryCode,
	vm::ptr<s32> language)
{
	sceNp.warning(
		"sceNpManagerGetAccountRegion(countryCode=*0x%x, language=*0x%x)",
...
error_code sceNpManagerGetContentRatingFlag(vm::ptr<s32> isRestricted,
	vm::ptr<s32> age)
{
	sceNp.trace("sceNpManagerGetContentRatingFlag(isRestricted=*0x%x, age=*0x%x)",
		isRestricted, age);
...
{
	sceNp.trace("sceNpManagerGetChatRestrictionFlag(isRestricted=*0x%x)",
		isRestricted);
...
error_code sceNpManagerGetCachedInfo(CellSysutilUserId userId,
	vm::ptr<SceNpManagerCacheParam> param)
{
	sceNp.warning("sceNpManagerGetCachedInfo(userId=%d, param=*0x%x)", userId,
		param);
...
error_code sceNpManagerRequestTicket(vm::cptr<SceNpId> npId,
	vm::cptr<char> serviceId,
	vm::cptr<void> cookie, u32 cookieSize,
	vm::cptr<char> entitlementId,
	u32 consumedCount)
{
	sceNp.warning(
		"sceNpManagerRequestTicket(npId=*0x%x, serviceId=%s, cookie=*0x%x, "
		"cookieSize=%d, entitlementId=%s, consumedCount=%d)",
	...
	nph.req_ticket(0x00020001, npId.get_ptr(), serviceId.get_ptr(),
		static_cast<const u8*>(cookie.get_ptr()), cookieSize,
		entitlementId.get_ptr(), consumedCount);

	return CELL_OK;
}

error_code sceNpManagerRequestTicket2(vm::cptr<SceNpId> npId,
	vm::cptr<SceNpTicketVersion> version,
	vm::cptr<char> serviceId,
	vm::cptr<void> cookie, u32 cookieSize,
	vm::cptr<char> entitlementId,
	u32 consumedCount)
{
	sceNp.warning(
		"sceNpManagerRequestTicket2(npId=*0x%x, version=*0x%x, serviceId=%s, "
		"cookie=*0x%x, cookieSize=%d, entitlementId=%s, consumedCount=%d)",
		npId, version, serviceId, cookie, cookieSize, entitlementId,
		consumedCount);
...
error_code sceNpManagerGetTicket(vm::ptr<void> buffer,
	vm::ptr<u32> bufferSize)
{
	sceNp.warning("sceNpManagerGetTicket(buffer=*0x%x, bufferSize=*0x%x)", buffer,
		bufferSize);
...
error_code sceNpManagerGetTicketParam(s32 paramId,
	vm::ptr<SceNpTicketParam> param)
{
...
error_code
sceNpManagerGetEntitlementIdList(vm::ptr<SceNpEntitlementId> entIdList,
	u32 entIdListNum)
{
	sceNp.todo(
		"sceNpManagerGetEntitlementIdList(entIdList=*0x%x, entIdListNum=%d)",
		entIdList, entIdListNum);
...
error_code sceNpManagerGetEntitlementById(vm::cptr<char> entId,
	vm::ptr<SceNpEntitlement> ent)
{
...
error_code sceNpManagerGetSigninId(vm::ptr<void> signInId)
{
	...
	std::memset(signInId.get_ptr(), 0,
		64); // signInId seems to be a 64 byte thing

	return CELL_OK;
}

error_code sceNpManagerSubSignin(CellSysutilUserId userId,
	vm::ptr<SceNpManagerSubSigninCallback> cb_func,
	vm::ptr<void> cb_arg, s32 flag)
{
	sceNp.todo(
		"sceNpManagerSubSignin(userId=%d, cb_func=*0x%x, cb_arg=*0x%x, flag=%d)",
		userId, cb_func, cb_arg, flag);
...
error_code check_attr_id(vm::cptr<SceNpMatchingAttr> attr)
{
	...
	if (!attr)
		return SCE_NP_MATCHING_ERROR_INVALID_ATTR_ID; // Satisfy compiler
...
error_code check_text(vm::cptr<char> text)
{
	...
	while (true)
	{
		...
		if (c < '\0')
		{
			if ((val & 0xe0) == 0xc0 && val > 0xc1 && (text[1] & 0xc0U) == 0x80)
			{
				text += 2;
			}
			else if ((val & 0xf0) == 0xe0 && (text[1] & 0xc0U) == 0x80 &&
					 (text[2] & 0xc0U) == 0x80)
			{
				text += 3;
			}
			...
			{
				if ((val & 0xf8) != 0xf0 || (text[1] & 0xc0U) != 0x80 ||
					(text[2] & 0xc0U) != 0x80 || (text[3] & 0xc0U) != 0x80)
				{
					return SCE_NP_UTIL_ERROR_INVALID_CHARACTER;
				}
				text += 4;
...
error_code sceNpMatchingCreateCtx(vm::ptr<SceNpId> npId,
	vm::ptr<SceNpMatchingHandler> handler,
	vm::ptr<void> arg, vm::ptr<u32> ctx_id)
{
	sceNp.warning("sceNpMatchingCreateCtx(npId=*0x%x, handler=*0x%x, arg=*0x%x, "
				  "ctx_id=*0x%x)",
		npId, handler, arg, ctx_id);
...
error_code sceNpMatchingGetResult(u32 ctx_id, u32 req_id, vm::ptr<void> buf,
	vm::ptr<u32> size, vm::ptr<s32> event)
{
	sceNp.warning("sceNpMatchingGetResult(ctx_id=%d, req_id=%d, buf=*0x%x, "
				  "size=*0x%x, event=*0x%x)",
		ctx_id, req_id, buf, size, event);
	...
	// const u64 id_check = static_cast<s64>(static_cast<s32>(req_id)) >> 0x17 &
	// 0x3f; if (id_check > 32 || (1ULL << id_check & 0x1f89ad040U) == 0) 	return
	// SCE_NP_MATCHING_ERROR_INVALID_REQ_ID;
}

error_code sceNpMatchingGetResultGUI(vm::ptr<void> buf, vm::ptr<u32> size,
	vm::ptr<s32> event)
{
	sceNp.warning("sceNpMatchingGetResultGUI(buf=*0x%x, size=*0x%x, event=*0x%x)",
		buf, size, event);
...
error_code matching_set_room_info(u32 ctx_id, vm::ptr<SceNpLobbyId> lobby_id,
	vm::ptr<SceNpRoomId> room_id,
	vm::ptr<SceNpMatchingAttr> attr,
	vm::ptr<u32> req_id, bool limit)
{
	auto& nph = g_fxo->get<named_thread<np::np_handler>>();
...
error_code sceNpMatchingSetRoomInfo(u32 ctx_id, vm::ptr<SceNpLobbyId> lobby_id,
	vm::ptr<SceNpRoomId> room_id,
	vm::ptr<SceNpMatchingAttr> attr,
	vm::ptr<u32> req_id)
{
	sceNp.warning("sceNpMatchingSetRoomInfo(ctx_id=%d, lobby_id=*0x%x, "
				  "room_id=*0x%x, attr=*0x%x, req_id=*0x%x)",
		ctx_id, lobby_id, room_id, attr, req_id);

	return matching_set_room_info(ctx_id, lobby_id, room_id, attr, req_id, true);
}

error_code sceNpMatchingSetRoomInfoNoLimit(u32 ctx_id,
	vm::ptr<SceNpLobbyId> lobby_id,
	vm::ptr<SceNpRoomId> room_id,
	vm::ptr<SceNpMatchingAttr> attr,
	vm::ptr<u32> req_id)
{
	sceNp.warning("sceNpMatchingSetRoomInfoNoLimit(ctx_id=%d, lobby_id=*0x%x, "
				  "room_id=*0x%x, attr=*0x%x, req_id=*0x%x)",
		ctx_id, lobby_id, room_id, attr, req_id);
...
error_code matching_get_room_info(u32 ctx_id, vm::ptr<SceNpLobbyId> lobby_id,
	vm::ptr<SceNpRoomId> room_id,
	vm::ptr<SceNpMatchingAttr> attr,
	vm::ptr<u32> req_id, bool limit)
{
...
error_code sceNpMatchingGetRoomInfo(u32 ctx_id, vm::ptr<SceNpLobbyId> lobby_id,
	vm::ptr<SceNpRoomId> room_id,
	vm::ptr<SceNpMatchingAttr> attr,
	vm::ptr<u32> req_id)
{
	sceNp.warning("sceNpMatchingGetRoomInfo(ctx_id=%d, lobby_id=*0x%x, "
				  "room_id=*0x%x, attr=*0x%x, req_id=*0x%x)",
		ctx_id, lobby_id, room_id, attr, req_id);
...
error_code sceNpMatchingGetRoomInfoNoLimit(u32 ctx_id,
	vm::ptr<SceNpLobbyId> lobby_id,
	vm::ptr<SceNpRoomId> room_id,
	vm::ptr<SceNpMatchingAttr> attr,
	vm::ptr<u32> req_id)
{
	sceNp.warning("sceNpMatchingGetRoomInfoNoLimit(ctx_id=%d, lobby_id=*0x%x, "
				  "room_id=*0x%x, attr=*0x%x, req_id=*0x%x)",
		ctx_id, lobby_id, room_id, attr, req_id);
...
error_code sceNpMatchingSetRoomSearchFlag(u32 ctx_id,
	vm::ptr<SceNpLobbyId> lobby_id,
	vm::ptr<SceNpRoomId> room_id,
	s32 flag, vm::ptr<u32> req_id)
{
	sceNp.warning("sceNpMatchingSetRoomSearchFlag(ctx_id=%d, lobby_id=*0x%x, "
				  "room_id=*0x%x, flag=%d, req_id=*0x%x)",
		ctx_id, lobby_id, room_id, flag, req_id);
	...
	if (!lobby_id || !room_id || !req_id ||
		(flag < SCE_NP_MATCHING_ROOM_SEARCH_FLAG_OPEN ||
			flag > SCE_NP_MATCHING_ROOM_SEARCH_FLAG_STEALTH))
		return SCE_NP_MATCHING_ERROR_INVALID_ARG;
...
error_code sceNpMatchingGetRoomSearchFlag(u32 ctx_id,
	vm::ptr<SceNpLobbyId> lobby_id,
	vm::ptr<SceNpRoomId> room_id,
	vm::ptr<u32> req_id)
{
	sceNp.warning("sceNpMatchingGetRoomSearchFlag(ctx_id=%d, lobby_id=*0x%x, "
				  "room_id=*0x%x, req_id=*0x%x)",
		ctx_id, lobby_id, room_id, req_id);
...
error_code matching_get_room_member_list(u32 ctx_id,
	vm::ptr<SceNpRoomId> room_id,
	vm::ptr<u32> buflen,
	vm::ptr<void> buf)
{
...
error_code sceNpMatchingGetRoomMemberListLocal(u32 ctx_id,
	vm::ptr<SceNpRoomId> room_id,
	vm::ptr<u32> buflen,
	vm::ptr<void> buf)
{
	sceNp.warning("sceNpMatchingGetRoomMemberListLocal(ctx_id=%d, room_id=*0x%x, "
				  "buflen=*0x%x, buf=*0x%x)",
		ctx_id, room_id, buflen, buf);

	return matching_get_room_member_list(ctx_id, room_id, buflen, buf);
}

// FUN_00014bdc
error_code check_room_list_params([[maybe_unused]] u32 ctx_id,
	vm::ptr<SceNpCommunicationId> communicationId,
	vm::ptr<SceNpMatchingReqRange> range,
	vm::ptr<SceNpMatchingSearchCondition> cond,
	vm::ptr<SceNpMatchingAttr> attribute,
	vm::ptr<SceNpMatchingGUIHandler> handler,
...
error_code matching_get_room_list(u32 ctx_id,
	vm::ptr<SceNpCommunicationId> communicationId,
	vm::ptr<SceNpMatchingReqRange> range,
	vm::ptr<SceNpMatchingSearchCondition> cond,
	vm::ptr<SceNpMatchingAttr> attr,
	vm::ptr<SceNpMatchingGUIHandler> handler,
	vm::ptr<void> arg, bool limit)
{
	auto& nph = g_fxo->get<named_thread<np::np_handler>>();
	...
	error_code err = check_room_list_params(ctx_id, communicationId, range, cond,
		attr, handler, 1, 0);
	if (err != CELL_OK)
		return err;

	return nph.get_room_list_gui(ctx_id, communicationId, range, cond, attr,
		handler, arg, limit);
}

error_code sceNpMatchingGetRoomListLimitGUI(
	u32 ctx_id, vm::ptr<SceNpCommunicationId> communicationId,
	vm::ptr<SceNpMatchingReqRange> range,
	vm::ptr<SceNpMatchingSearchCondition> cond, vm::ptr<SceNpMatchingAttr> attr,
	vm::ptr<SceNpMatchingGUIHandler> handler, vm::ptr<void> arg)
{
	sceNp.warning(
		"sceNpMatchingGetRoomListLimitGUI(ctx_id=%d, communicationId=*0x%x, "
		"range=*0x%x, cond=*0x%x, attr=*0x%x, handler=*0x%x, arg=*0x%x)",
		ctx_id, communicationId, range, cond, attr, handler, arg);

	return matching_get_room_list(ctx_id, communicationId, range, cond, attr,
		handler, arg, true);
}

error_code sceNpMatchingKickRoomMember(u32 ctx_id,
	vm::cptr<SceNpRoomId> room_id,
	vm::cptr<SceNpId> user_id,
	vm::ptr<u32> req_id)
{
	sceNp.todo("sceNpMatchingKickRoomMember(ctx_id=%d, room_id=*0x%x, "
			   "user_id=*0x%x, req_id=*0x%x)",
		ctx_id, room_id, user_id, req_id);
...
error_code sceNpMatchingKickRoomMemberWithOpt(u32 ctx_id,
	vm::cptr<SceNpRoomId> room_id,
	vm::cptr<SceNpId> user_id,
	vm::cptr<void> opt, s32 opt_len,
	vm::ptr<u32> req_id)
{
	sceNp.todo("sceNpMatchingKickRoomMemberWithOpt(ctx_id=%d, room_id=*0x%x, "
			   "user_id=*0x%x, opt=*0x%x, opt_len=%d, req_id=*0x%x)",
		ctx_id, room_id, user_id, opt, opt_len, req_id);
...
error_code sceNpMatchingQuickMatchGUI(
	u32 ctx_id, vm::cptr<SceNpCommunicationId> communicationId,
	vm::cptr<SceNpMatchingSearchCondition> cond, s32 available_num, s32 timeout,
	vm::ptr<SceNpMatchingGUIHandler> handler, vm::ptr<void> arg)
{
	sceNp.warning(
		"sceNpMatchingQuickMatchGUI(ctx_id=%d, communicationId=*0x%x, "
		"cond=*0x%x, available_num=%d, timeout=%d, handler=*0x%x, arg=*0x%x)",
	...
	for (auto con = cond; !!con; con = con->next)
	{
		...
		if (con->comp_type ==
			1) // weird, should be != SCE_NP_MATCHING_CONDITION_TYPE_VALUE
	...
	return nph.quickmatch_gui(ctx_id, communicationId, cond, available_num,
		timeout, handler, arg);
}

error_code sceNpMatchingSendInvitationGUI(
	u32 ctx_id, vm::cptr<SceNpRoomId> room_id,
	vm::cptr<SceNpCommunicationId> communicationId, vm::cptr<SceNpId> dsts,
	s32 num, s32 slot_type, vm::cptr<char> subject, vm::cptr<char> body,
	sys_memory_container_t container, vm::ptr<SceNpMatchingGUIHandler> handler,
	vm::ptr<void> arg)
{
	sceNp.todo("sceNpMatchingSendInvitationGUI(ctx_id=%d, room_id=*0x%x, "
			   "communicationId=*0x%x, dsts=*0x%x, num=%d, slot_type=%d, "
			   "subject=%s, body=%s, container=%d, handler=*0x%x, arg=*0x%x)",
		ctx_id, room_id, communicationId, dsts, num, slot_type, subject,
		body, container, handler, arg);
	...
	if (!room_id || !communicationId || !subject ||
		!body) // TODO: || (in_stack_0000007c != 0)
		return SCE_NP_MATCHING_ERROR_INVALID_ARG;
	...
	if (slot_type < SCE_NP_MATCHING_ROOM_SLOT_TYPE_PUBLIC ||
		slot_type > SCE_NP_MATCHING_ROOM_SLOT_TYPE_PRIVATE)
		return SCE_NP_MATCHING_ERROR_INVALID_ARG;
...
error_code sceNpMatchingAcceptInvitationGUI(
	u32 ctx_id, vm::cptr<SceNpCommunicationId> communicationId,
	sys_memory_container_t container, vm::ptr<SceNpMatchingGUIHandler> handler,
	vm::ptr<void> arg)
{
	sceNp.todo("sceNpMatchingAcceptInvitationGUI(ctx_id=%d, "
			   "communicationId=*0x%x, container=%d, handler=*0x%x, arg=*0x%x)",
		ctx_id, communicationId, container, handler, arg);
...
error_code matching_create_room(u32 ctx_id,
	vm::cptr<SceNpCommunicationId> communicationId,
	vm::cptr<SceNpMatchingAttr> attr,
	vm::ptr<SceNpMatchingGUIHandler> handler,
	vm::ptr<void> arg)
{
	auto& nph = g_fxo->get<named_thread<np::np_handler>>();
...
error_code sceNpMatchingCreateRoomGUI(
	u32 ctx_id, vm::cptr<SceNpCommunicationId> communicationId,
	vm::cptr<SceNpMatchingAttr> attr, vm::ptr<SceNpMatchingGUIHandler> handler,
	vm::ptr<void> arg)
{
	sceNp.warning("sceNpMatchingCreateRoomGUI(ctx_id=%d, communicationId=*0x%x, "
				  "attr=*0x%x, handler=*0x%x, arg=*0x%x)",
		ctx_id, communicationId, attr, handler, arg);

	return matching_create_room(ctx_id, communicationId, attr, handler, arg);
}

error_code matching_join_room(u32 ctx_id, vm::ptr<SceNpRoomId> room_id,
	vm::ptr<SceNpMatchingGUIHandler> handler,
	vm::ptr<void> arg)
{
	auto& nph = g_fxo->get<named_thread<np::np_handler>>();
...
error_code sceNpMatchingJoinRoomGUI(u32 ctx_id, vm::ptr<SceNpRoomId> room_id,
	vm::ptr<SceNpMatchingGUIHandler> handler,
	vm::ptr<void> arg)
{
	sceNp.warning("sceNpMatchingJoinRoomGUI(ctx_id=%d, room_id=*0x%x, "
				  "handler=*0x%x, arg=*0x%x)",
		ctx_id, room_id, handler, arg);

	return matching_join_room(ctx_id, room_id, handler, arg);
}

error_code sceNpMatchingLeaveRoom(u32 ctx_id, vm::cptr<SceNpRoomId> room_id,
	vm::ptr<u32> req_id)
{
	sceNp.warning(
		"sceNpMatchingLeaveRoom(ctx_id=%d, room_id=*0x%x, req_id=*0x%x)", ctx_id,
		room_id, req_id);
...
error_code sceNpMatchingSearchJoinRoomGUI(
	u32 ctx_id, vm::cptr<SceNpCommunicationId> communicationId,
	vm::cptr<SceNpMatchingSearchCondition> cond,
	vm::cptr<SceNpMatchingAttr> attr, vm::ptr<SceNpMatchingGUIHandler> handler,
	vm::ptr<void> arg)
{
	sceNp.warning(
		"sceNpMatchingSearchJoinRoomGUI(ctx_id=%d, communicationId=*0x%x, "
		"cond=*0x%x, attr=*0x%x, handler=*0x%x, arg=*0x%x)",
...
error_code sceNpMatchingGrantOwnership(u32 ctx_id,
	vm::cptr<SceNpRoomId> room_id,
	vm::cptr<SceNpId> user_id,
	vm::ptr<u32> req_id)
{
	sceNp.todo("sceNpMatchingGrantOwnership(ctx_id=%d, room_id=*0x%x, "
			   "user_id=*0x%x, req_id=*0x%x)",
		ctx_id, room_id, user_id, req_id);
...
error_code sceNpProfileCallGui(vm::cptr<SceNpId> npid,
	vm::ptr<SceNpProfileResultHandler> handler,
	vm::ptr<void> userArg, u64 options)
{
	sceNp.todo("sceNpProfileCallGui(npid=*0x%x, handler=*0x%x, userArg=*0x%x, "
			   "options=0x%x)",
		npid, handler, userArg, options);
...
error_code sceNpProfileAbortGui()
{
	...
	if (!nph.is_NP_init)
	{
		return SCE_NP_FRIENDLIST_ERROR_NOT_INITIALIZED; // Not
...
error_code
sceNpScoreCreateTitleCtx(vm::cptr<SceNpCommunicationId> communicationId,
	vm::cptr<SceNpCommunicationPassphrase> passphrase,
	vm::cptr<SceNpId> selfNpId)
{
	sceNp.warning("sceNpScoreCreateTitleCtx(communicationId=*0x%x(%s), "
				  "passphrase=*0x%x, selfNpId=*0x%x)",
		communicationId,
		communicationId ? np::communication_id_to_string(*communicationId).c_str() : "",
		passphrase, selfNpId);
...
{
	sceNp.warning("sceNpScoreSetPlayerCharacterId(ctxId=%d, pcId=%d)", ctxId,
		pcId);
...
{
	sceNp.warning("sceNpScoreWaitAsync(transId=%d, result=*0x%x)", transId,
		result);
...
{
	sceNp.warning("sceNpScorePollAsync(transId=%d, result=*0x%x)", transId,
		result);
...
std::pair<std::optional<error_code>, shared_ptr<score_transaction_ctx>>
get_score_transaction_context(s32 transId, bool reset_transaction = true)
{
	...
	if (reset_transaction)
	{
		...
		// Unsure about the actual behaviour, only one game does this afaik(Marvel
		// vs Capcom Origins) For now we just clean the context and pretend it's a
		// new one
...
error_code scenp_score_get_board_info(s32 transId, SceNpScoreBoardId boardId,
	vm::ptr<SceNpScoreBoardInfo> boardInfo,
	vm::ptr<void> option, bool async)
{
	auto& nph = g_fxo->get<named_thread<np::np_handler>>();
...
error_code sceNpScoreGetBoardInfo(s32 transId, SceNpScoreBoardId boardId,
	vm::ptr<SceNpScoreBoardInfo> boardInfo,
	vm::ptr<void> option)
{
	sceNp.warning("sceNpScoreGetBoardInfo(transId=%d, boardId=%d, "
				  "boardInfo=*0x%x, option=*0x%x)",
		transId, boardId, boardInfo, option);

	return scenp_score_get_board_info(transId, boardId, boardInfo, option, false);
}

error_code sceNpScoreGetBoardInfoAsync(s32 transId, SceNpScoreBoardId boardId,
	vm::ptr<SceNpScoreBoardInfo> boardInfo,
	s32 prio, vm::ptr<void> option)
{
	sceNp.warning("sceNpScoreGetBoardInfo(transId=%d, boardId=%d, "
				  "boardInfo=*0x%x, prio=%d, option=*0x%x)",
		transId, boardId, boardInfo, prio, option);

	return scenp_score_get_board_info(transId, boardId, boardInfo, option, true);
}

error_code scenp_score_record_score(s32 transId, SceNpScoreBoardId boardId,
	...
	vm::ptr<SceNpScoreRecordOptParam> option,
	bool async)
{
	auto& nph = g_fxo->get<named_thread<np::np_handler>>();
	...
	nph.record_score(trans_ctx, boardId, score, scoreComment, data, data_size,
		tmpRank, async);
...
error_code sceNpScoreRecordScore(s32 transId, SceNpScoreBoardId boardId,
	SceNpScoreValue score,
	vm::cptr<SceNpScoreComment> scoreComment,
	vm::cptr<SceNpScoreGameInfo> gameInfo,
	vm::ptr<SceNpScoreRankNumber> tmpRank,
...
{
	sceNp.warning(
		"sceNpScoreRecordScore(transId=%d, boardId=%d, score=%d, "
		"scoreComment=*0x%x, gameInfo=*0x%x, tmpRank=*0x%x, option=*0x%x)",
		transId, boardId, score, scoreComment, gameInfo, tmpRank, option);

	return scenp_score_record_score(transId, boardId, score, scoreComment,
		gameInfo, tmpRank, option, false);
}

error_code
sceNpScoreRecordScoreAsync(s32 transId, SceNpScoreBoardId boardId,
	SceNpScoreValue score,
	vm::cptr<SceNpScoreComment> scoreComment,
	vm::cptr<SceNpScoreGameInfo> gameInfo,
	vm::ptr<SceNpScoreRankNumber> tmpRank, s32 prio,
	vm::ptr<SceNpScoreRecordOptParam> option)
{
	sceNp.warning("sceNpScoreRecordScoreAsync(transId=%d, boardId=%d, score=%d, "
				  "scoreComment=*0x%x, gameInfo=*0x%x, tmpRank=*0x%x, prio=%d, "
				  "option=*0x%x)",
		transId, boardId, score, scoreComment, gameInfo, tmpRank, prio,
		option);

	return scenp_score_record_score(transId, boardId, score, scoreComment,
		gameInfo, tmpRank, option, true);
}

error_code scenp_score_record_game_data(s32 transId, SceNpScoreBoardId boardId,
	SceNpScoreValue score, u32 totalSize,
	u32 sendSize, vm::cptr<void> data,
	vm::ptr<void> /* option */,
	bool async)
{
	auto& nph = g_fxo->get<named_thread<np::np_handler>>();
	...
	nph.record_score_data(trans_ctx, boardId, score, totalSize, sendSize,
		static_cast<const u8*>(data.get_ptr()), async);
...
error_code sceNpScoreRecordGameData(s32 transId, SceNpScoreBoardId boardId,
	SceNpScoreValue score, u32 totalSize,
	u32 sendSize, vm::cptr<void> data,
	vm::ptr<void> option)
{
	sceNp.warning("sceNpScoreRecordGameData(transId=%d, boardId=%d, score=%d, "
				  "totalSize=%d, sendSize=%d, data=*0x%x, option=*0x%x)",
		transId, boardId, score, totalSize, sendSize, data, option);

	return scenp_score_record_game_data(transId, boardId, score, totalSize,
		sendSize, data, option, false);
}

error_code sceNpScoreRecordGameDataAsync(s32 transId, SceNpScoreBoardId boardId,
	SceNpScoreValue score, u32 totalSize,
	u32 sendSize, vm::cptr<void> data,
	s32 prio, vm::ptr<void> option)
{
	sceNp.warning(
		"sceNpScoreRecordGameDataAsync(transId=%d, boardId=%d, score=%d, "
		"totalSize=%d, sendSize=%d, data=*0x%x, prio=%d, option=*0x%x)",
		transId, boardId, score, totalSize, sendSize, data, prio, option);

	return scenp_score_record_game_data(transId, boardId, score, totalSize,
		sendSize, data, option, true);
}

error_code scenp_score_get_game_data(s32 transId, SceNpScoreBoardId boardId,
	vm::cptr<SceNpId> npId,
	vm::ptr<u32> totalSize, u32 recvSize,
	vm::ptr<void> data,
	vm::ptr<void> /* option */, bool async)
{
	auto& nph = g_fxo->get<named_thread<np::np_handler>>();
	...
	nph.get_score_data(trans_ctx, boardId, *npId, totalSize, recvSize, data,
		async);
...
error_code sceNpScoreGetGameData(s32 transId, SceNpScoreBoardId boardId,
	vm::cptr<SceNpId> npId, vm::ptr<u32> totalSize,
	u32 recvSize, vm::ptr<void> data,
	vm::ptr<void> option)
{
	sceNp.warning("sceNpScoreGetGameData(transId=%d, boardId=%d, npId=*0x%x, "
				  "totalSize=*0x%x, recvSize=%d, data=*0x%x, option=*0x%x)",
		transId, boardId, npId, totalSize, recvSize, data, option);

	return scenp_score_get_game_data(transId, boardId, npId, totalSize, recvSize,
		data, option, false);
}

error_code sceNpScoreGetGameDataAsync(s32 transId, SceNpScoreBoardId boardId,
	vm::cptr<SceNpId> npId,
	vm::ptr<u32> totalSize, u32 recvSize,
	vm::ptr<void> data, s32 prio,
	vm::ptr<void> option)
{
	sceNp.warning(
		"sceNpScoreGetGameDataAsync(transId=%d, boardId=%d, npId=*0x%x, "
		"totalSize=*0x%x, recvSize=%d, data=*0x%x, prio=%d, option=*0x%x)",
...
template <typename T>
error_code scenp_score_get_ranking_by_npid(
	s32 transId, SceNpScoreBoardId boardId, T npIdArray, u32 npIdArraySize,
	vm::ptr<SceNpScorePlayerRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
	vm::ptr<void> infoArray, u32 infoArraySize, u32 arrayNum,
	...
	bool async)
{
	...
	if (commentArray &&
		commentArraySize != (arrayNum * sizeof(SceNpScoreComment)))
	{
		return SCE_NP_COMMUNITY_ERROR_INVALID_ALIGNMENT;
	}

	if (infoArray && infoArraySize != (arrayNum * sizeof(SceNpScoreGameInfo)) &&
		infoArraySize != (arrayNum * sizeof(SceNpScoreVariableSizeGameInfo)))
	{
		return SCE_NP_COMMUNITY_ERROR_INVALID_ALIGNMENT;
	}
	...
	const bool deprecated =
		(rankArraySize ==
			(arrayNum * sizeof(SceNpScorePlayerRankData_deprecated)));
	...
	static constexpr bool is_npidpcid =
		std::is_same_v<T, vm::cptr<SceNpScoreNpIdPcId>>;
	static_assert(
		is_npid || is_npidpcid,
		"T should be vm::cptr<SceNpId> or vm::cptr<SceNpScoreNpIdPcId>");
	...
	else if constexpr (is_npidpcid)
	{
		...
		{
			npid_vec.push_back(
				std::make_pair(npIdArray[index].npId, npIdArray[index].pcId));
		}
	}

	nph.get_score_npid(trans_ctx, boardId, npid_vec, rankArray, rankArraySize,
		commentArray, commentArraySize, infoArray, infoArraySize,
		arrayNum, lastSortDate, totalRecord, async, deprecated);
...
error_code sceNpScoreGetRankingByNpId(
	s32 transId, SceNpScoreBoardId boardId, vm::cptr<SceNpId> npIdArray,
	u32 npIdArraySize, vm::ptr<SceNpScorePlayerRankData> rankArray,
	u32 rankArraySize, vm::ptr<SceNpScoreComment> commentArray,
	u32 commentArraySize, vm::ptr<void> infoArray, u32 infoArraySize,
	u32 arrayNum, vm::ptr<CellRtcTick> lastSortDate,
	vm::ptr<SceNpScoreRankNumber> totalRecord, vm::ptr<void> option)
{
	sceNp.warning(
		"sceNpScoreGetRankingByNpId(transId=%d, boardId=%d, npIdArray=*0x%x, "
		"npIdArraySize=%d, rankArray=*0x%x, rankArraySize=%d, "
		"commentArray=*0x%x, commentArraySize=%d, infoArray=*0x%x, "
		"infoArraySize=%d, arrayNum=%d, lastSortDate=*0x%x, totalRecord=*0x%x, "
		"option=*0x%x)",
		transId, boardId, npIdArray, npIdArraySize, rankArray, rankArraySize,
		commentArray, commentArraySize, infoArray, infoArraySize, arrayNum,
		lastSortDate, totalRecord, option);

	return scenp_score_get_ranking_by_npid(
		transId, boardId, npIdArray, npIdArraySize, rankArray, rankArraySize,
		commentArray, commentArraySize, infoArray, infoArraySize, arrayNum,
		lastSortDate, totalRecord, option, false);
}

error_code sceNpScoreGetRankingByNpIdAsync(
	s32 transId, SceNpScoreBoardId boardId, vm::cptr<SceNpId> npIdArray,
	u32 npIdArraySize, vm::ptr<SceNpScorePlayerRankData> rankArray,
	u32 rankArraySize, vm::ptr<SceNpScoreComment> commentArray,
	u32 commentArraySize, vm::ptr<void> infoArray, u32 infoArraySize,
...
{
	sceNp.warning(
		"sceNpScoreGetRankingByNpIdAsync(transId=%d, boardId=%d, "
		"npIdArray=*0x%x, npIdArraySize=%d, rankArray=*0x%x, rankArraySize=%d, "
		"commentArray=*0x%x, commentArraySize=%d, infoArray=*0x%x, "
		"infoArraySize=%d, arrayNum=%d, lastSortDate=*0x%x, totalRecord=*0x%x, "
		"prio=%d, option=*0x%x)",
		transId, boardId, npIdArray, npIdArraySize, rankArray, rankArraySize,
...
error_code scenp_score_get_ranking_by_range(
	s32 transId, SceNpScoreBoardId boardId,
	SceNpScoreRankNumber startSerialRank, vm::ptr<SceNpScoreRankData> rankArray,
	u32 rankArraySize, vm::ptr<SceNpScoreComment> commentArray,
	u32 commentArraySize, vm::ptr<void> infoArray, u32 infoArraySize,
	u32 arrayNum, vm::ptr<CellRtcTick> lastSortDate,
	vm::ptr<SceNpScoreRankNumber> totalRecord, vm::ptr<void> option,
	bool async)
{
	...
	const bool deprecated =
		(rankArraySize == (arrayNum * sizeof(SceNpScoreRankData_deprecated)));
	...
	nph.get_score_range(trans_ctx, boardId, startSerialRank, rankArray,
		rankArraySize, commentArray, commentArraySize, infoArray,
		infoArraySize, arrayNum, lastSortDate, totalRecord, async,
		deprecated);
...
error_code sceNpScoreGetRankingByRange(
	s32 transId, SceNpScoreBoardId boardId,
	SceNpScoreRankNumber startSerialRank, vm::ptr<SceNpScoreRankData> rankArray,
	u32 rankArraySize, vm::ptr<SceNpScoreComment> commentArray,
	u32 commentArraySize, vm::ptr<void> infoArray, u32 infoArraySize,
...
{
	sceNp.warning(
		"sceNpScoreGetRankingByRange(transId=%d, boardId=%d, startSerialRank=%d, "
		"rankArray=*0x%x, rankArraySize=%d, commentArray=*0x%x, "
		"commentArraySize=%d, infoArray=*0x%x, infoArraySize=%d, "
		"arrayNum=%d, lastSortDate=*0x%x, totalRecord=*0x%x, option=*0x%x)",
		transId, boardId, startSerialRank, rankArray, rankArraySize, commentArray,
		commentArraySize, infoArray, infoArraySize, arrayNum, lastSortDate,
		totalRecord, option);

	return scenp_score_get_ranking_by_range(
		transId, boardId, startSerialRank, rankArray, rankArraySize, commentArray,
		commentArraySize, infoArray, infoArraySize, arrayNum, lastSortDate,
		totalRecord, option, false);
}

error_code sceNpScoreGetRankingByRangeAsync(
	s32 transId, SceNpScoreBoardId boardId,
	SceNpScoreRankNumber startSerialRank, vm::ptr<SceNpScoreRankData> rankArray,
	u32 rankArraySize, vm::ptr<SceNpScoreComment> commentArray,
	u32 commentArraySize, vm::ptr<void> infoArray, u32 infoArraySize,
	...
	vm::ptr<SceNpScoreRankNumber> totalRecord, s32 prio, vm::ptr<void> option)
{
	sceNp.warning("sceNpScoreGetRankingByRangeAsync(transId=%d, boardId=%d, "
				  "startSerialRank=%d, rankArray=*0x%x, rankArraySize=%d, "
				  "commentArray=*0x%x, commentArraySize=%d, infoArray=*0x%x, "
				  "infoArraySize=%d, arrayNum=%d, lastSortDate=*0x%x, "
				  "totalRecord=*0x%x, prio=%d, option=*0x%x)",
		transId, boardId, startSerialRank, rankArray, rankArraySize,
		commentArray, commentArraySize, infoArray, infoArraySize,
...
error_code scenp_score_get_friends_ranking(
	s32 transId, SceNpScoreBoardId boardId, s32 includeSelf,
	vm::ptr<SceNpScoreRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
	vm::ptr<SceNpScoreGameInfo> infoArray, u32 infoArraySize, u32 arrayNum,
	vm::ptr<CellRtcTick> lastSortDate,
	vm::ptr<SceNpScoreRankNumber> totalRecord, vm::ptr<void> option,
	bool async)
{
	...
	if (arrayNum > SCE_NP_SCORE_MAX_SELECTED_FRIENDS_NUM ||
		option) // option check at least until fw 4.71
	{
		return SCE_NP_COMMUNITY_ERROR_INVALID_ARGUMENT;
	}
	...
	nph.get_score_friend(trans_ctx, boardId, includeSelf, rankArray,
		rankArraySize, commentArray, commentArraySize, infoArray,
...
error_code sceNpScoreGetFriendsRanking(
	s32 transId, SceNpScoreBoardId boardId, s32 includeSelf,
	vm::ptr<SceNpScoreRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
	vm::ptr<SceNpScoreGameInfo> infoArray, u32 infoArraySize, u32 arrayNum,
...
{
	sceNp.warning(
		"sceNpScoreGetFriendsRanking(transId=%d, boardId=%d, includeSelf=%d, "
		"rankArray=*0x%x, rankArraySize=%d, commentArray=*0x%x, "
		"commentArraySize=%d, infoArray=*0x%x, infoArraySize=%d, "
		"arrayNum=%d, lastSortDate=*0x%x, totalRecord=*0x%x, option=*0x%x)",
	...
	return scenp_score_get_friends_ranking(
		transId, boardId, includeSelf, rankArray, rankArraySize, commentArray,
		commentArraySize, infoArray, infoArraySize, arrayNum, lastSortDate,
		totalRecord, option, false);
}

error_code sceNpScoreGetFriendsRankingAsync(
	s32 transId, SceNpScoreBoardId boardId, s32 includeSelf,
	vm::ptr<SceNpScoreRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
	vm::ptr<SceNpScoreGameInfo> infoArray, u32 infoArraySize, u32 arrayNum,
...
{
	sceNp.warning(
		"sceNpScoreGetFriendsRankingAsync(transId=%d, boardId=%d, "
		"includeSelf=%d, rankArray=*0x%x, rankArraySize=%d, commentArray=*0x%x, "
		"commentArraySize=%d, infoArray=*0x%x, infoArraySize=%d, "
		"arrayNum=%d, lastSortDate=*0x%x, totalRecord=*0x%x, prio=%d, "
		"option=*0x%x)",
		transId, boardId, includeSelf, rankArray, rankArraySize, commentArray,
		commentArraySize, infoArray, infoArraySize, arrayNum, lastSortDate,
...
error_code scenp_score_censor_comment(s32 transId, vm::cptr<char> comment,
	vm::ptr<void> option, bool async)
{
	auto& nph = g_fxo->get<named_thread<np::np_handler>>();
	...
	if (strlen(comment.get_ptr()) > SCE_NP_SCORE_CENSOR_COMMENT_MAXLEN ||
		option) // option check at least until fw 4.71
	{
...
error_code sceNpScoreCensorComment(s32 transId, vm::cptr<char> comment,
	vm::ptr<void> option)
{
	sceNp.todo("sceNpScoreCensorComment(transId=%d, comment=%s, option=*0x%x)",
		transId, comment, option);

	return scenp_score_censor_comment(transId, comment, option, false);
}

error_code sceNpScoreCensorCommentAsync(s32 transId, vm::cptr<char> comment,
	s32 prio, vm::ptr<void> option)
{
	sceNp.todo("sceNpScoreCensorCommentAsync(transId=%d, comment=%s, prio=%d, "
			   "option=*0x%x)",
		transId, comment, prio, option);

	return scenp_score_censor_comment(transId, comment, option, true);
}

error_code scenp_score_sanitize_comment(s32 transId, vm::cptr<char> comment,
	vm::ptr<char> sanitizedComment,
	vm::ptr<void> /* option */,
	bool async)
{
	if (!sanitizedComment)
...
error_code sceNpScoreSanitizeComment(s32 transId, vm::cptr<char> comment,
	vm::ptr<char> sanitizedComment,
	vm::ptr<void> option)
{
	sceNp.warning("sceNpScoreSanitizeComment(transId=%d, comment=%s, "
				  "sanitizedComment=*0x%x, option=*0x%x)",
		transId, comment, sanitizedComment, option);

	return scenp_score_sanitize_comment(transId, comment, sanitizedComment,
		option, false);
}

error_code sceNpScoreSanitizeCommentAsync(s32 transId, vm::cptr<char> comment,
	vm::ptr<char> sanitizedComment,
	s32 prio, vm::ptr<void> option)
{
	sceNp.warning("sceNpScoreSanitizeCommentAsync(transId=%d, comment=%s, "
				  "sanitizedComment=*0x%x, prio=%d, option=*0x%x)",
		transId, comment, sanitizedComment, prio, option);

	return scenp_score_sanitize_comment(transId, comment, sanitizedComment,
		option, true);
}

error_code sceNpScoreGetRankingByNpIdPcId(
	s32 transId, SceNpScoreBoardId boardId,
	vm::cptr<SceNpScoreNpIdPcId> idArray, u32 idArraySize,
	vm::ptr<SceNpScorePlayerRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
	vm::ptr<void> infoArray, u32 infoArraySize, u32 arrayNum,
	vm::ptr<CellRtcTick> lastSortDate,
...
{
	sceNp.warning(
		"sceNpScoreGetRankingByNpIdPcId(transId=%d, boardId=%d, idArray=*0x%x, "
		"idArraySize=%d, rankArray=*0x%x, rankArraySize=%d, commentArray=*0x%x, "
		"commentArraySize=%d, infoArray=*0x%x, "
		"infoArraySize=%d, arrayNum=%d, lastSortDate=*0x%x, totalRecord=*0x%x, "
		"option=*0x%x)",
		transId, boardId, idArray, idArraySize, rankArray, rankArraySize,
		commentArray, commentArraySize, infoArray, infoArraySize, arrayNum,
		lastSortDate, totalRecord, option);
...
error_code sceNpScoreGetRankingByNpIdPcIdAsync(
	s32 transId, SceNpScoreBoardId boardId,
	vm::cptr<SceNpScoreNpIdPcId> idArray, u32 idArraySize,
	vm::ptr<SceNpScorePlayerRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
...
{
	sceNp.warning(
		"sceNpScoreGetRankingByNpIdPcIdAsync(transId=%d, boardId=%d, "
		"idArray=*0x%x, idArraySize=%d, rankArray=*0x%x, rankArraySize=%d, "
		"commentArray=*0x%x, commentArraySize=%d, infoArray=*0x%x, "
		"infoArraySize=%d, arrayNum=%d, lastSortDate=*0x%x, totalRecord=*0x%x, "
		"prio=%d, option=*0x%x)",
		transId, boardId, idArray, idArraySize, rankArray, rankArraySize,
...
error_code sceNpScoreGetClansMembersRankingByNpId(
	s32 transId, SceNpClanId clanId, SceNpScoreBoardId boardId,
	vm::cptr<SceNpId> idArray, u32 idArraySize,
	vm::ptr<SceNpScorePlayerRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
	vm::ptr<SceNpScoreGameInfo> infoArray, u32 infoArraySize,
	vm::ptr<SceNpScoreClansMemberDescription> descriptArray,
	u32 descriptArraySize, u32 arrayNum,
	vm::ptr<SceNpScoreClanBasicInfo> clanInfo,
	vm::ptr<CellRtcTick> lastSortDate,
	vm::ptr<SceNpScoreRankNumber> totalRecord, vm::ptr<void> option)
{
	sceNp.todo("sceNpScoreGetClansMembersRankingByNpId(transId=%d, clanId=%d, "
			   "boardId=%d, idArray=*0x%x, idArraySize=%d, rankArray=*0x%x, "
			   "rankArraySize=%d, commentArray=*0x%x, commentArraySize=%d, "
			   "infoArray=*0x%x, infoArraySize=%d, descriptArray=*0x%x, "
			   "descriptArraySize=%d, arrayNum=%d, clanInfo=*0x%x, "
			   "lastSortDate=*0x%x, totalRecord=*0x%x, option=*0x%x)",
		transId, clanId, boardId, idArray, idArraySize, rankArray,
		rankArraySize, commentArray, commentArraySize, infoArray,
		infoArraySize, descriptArray, descriptArraySize, arrayNum,
		clanInfo, lastSortDate, totalRecord, option);
...
error_code sceNpScoreGetClansMembersRankingByNpIdAsync(
	s32 transId, SceNpClanId clanId, SceNpScoreBoardId boardId,
	vm::cptr<SceNpId> idArray, u32 idArraySize,
	vm::ptr<SceNpScorePlayerRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
...
{
	sceNp.todo(
		"sceNpScoreGetClansMembersRankingByNpIdAsync(transId=%d, clanId=%d, "
		"boardId=%d, idArray=*0x%x, idArraySize=%d, rankArray=*0x%x, "
		"rankArraySize=%d, commentArray=*0x%x, commentArraySize=%d, "
		"infoArray=*0x%x, infoArraySize=%d, descriptArray=*0x%x, "
		"descriptArraySize=%d, arrayNum=%d, clanInfo=*0x%x, lastSortDate=*0x%x, "
		"totalRecord=*0x%x, prio=%d, option=*0x%x)",
		transId, clanId, boardId, idArray, idArraySize, rankArray, rankArraySize,
		commentArray, commentArraySize, infoArray, infoArraySize, descriptArray,
		descriptArraySize, arrayNum, clanInfo, lastSortDate, totalRecord, prio,
		option);
...
error_code sceNpScoreGetClansMembersRankingByNpIdPcId(
	s32 transId, SceNpClanId clanId, SceNpScoreBoardId boardId,
	vm::cptr<SceNpId> idArray, u32 idArraySize,
	vm::ptr<SceNpScorePlayerRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
...
{
	sceNp.todo(
		"sceNpScoreGetClansMembersRankingByNpIdPcId(transId=%d, clanId=%d, "
		"boardId=%d, idArray=*0x%x, idArraySize=%d, rankArray=*0x%x, "
		"rankArraySize=%d, commentArray=*0x%x, commentArraySize=%d, "
		"infoArray=*0x%x, infoArraySize=%d, descriptArray=*0x%x, "
		"descriptArraySize=%d, arrayNum=%d, clanInfo=*0x%x, lastSortDate=*0x%x, "
		"totalRecord=*0x%x, option=*0x%x)",
		transId, clanId, boardId, idArray, idArraySize, rankArray, rankArraySize,
		commentArray, commentArraySize, infoArray, infoArraySize, descriptArray,
		descriptArraySize, arrayNum, clanInfo, lastSortDate, totalRecord, option);
...
error_code sceNpScoreGetClansMembersRankingByNpIdPcIdAsync(
	s32 transId, SceNpClanId clanId, SceNpScoreBoardId boardId,
	vm::cptr<SceNpId> idArray, u32 idArraySize,
	vm::ptr<SceNpScorePlayerRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
...
{
	sceNp.todo(
		"sceNpScoreGetClansMembersRankingByNpIdPcIdAsync(transId=%d, clanId=%d, "
		"boardId=%d, idArray=*0x%x, idArraySize=%d, rankArray=*0x%x, "
		"rankArraySize=%d, commentArray=*0x%x, commentArraySize=%d, "
		"infoArray=*0x%x, infoArraySize=%d, descriptArray=*0x%x, "
		"descriptArraySize=%d, arrayNum=%d, clanInfo=*0x%x, lastSortDate=*0x%x, "
...
error_code sceNpScoreGetClansRankingByRange(
	s32 transId, SceNpScoreClansBoardId clanBoardId,
	SceNpScoreRankNumber startSerialRank,
	vm::ptr<SceNpScoreClanIdRankData> rankArray, u32 rankArraySize,
	vm::ptr<void> reserved1, u32 reservedSize1, vm::ptr<void> reserved2,
	u32 reservedSize2, u32 arrayNum, vm::ptr<CellRtcTick> lastSortDate,
	vm::ptr<SceNpScoreRankNumber> totalRecord, vm::ptr<void> option)
{
	sceNp.todo(
		"sceNpScoreGetClansRankingByRange(transId=%d, clanBoardId=%d, "
		"startSerialRank=%d, rankArray=*0x%x, rankArraySize=%d, reserved1=*0x%x, "
		"reservedSize1=%d, reserved2=*0x%x, reservedSize2=%d, "
		"arrayNum=%d, lastSortDate=*0x%x, totalRecord=*0x%x, option=*0x%x)",
		transId, clanBoardId, startSerialRank, rankArray, rankArraySize,
		reserved1, reservedSize1, reserved2, reservedSize2, arrayNum,
		lastSortDate, totalRecord, option);
	...
	if (!startSerialRank || reserved1 || reservedSize1 || reserved2 ||
		reservedSize2 ||
		option) // reserved and option checks at least until fw 4.71
	{
		return SCE_NP_COMMUNITY_ERROR_INVALID_ARGUMENT;
	}
...
error_code sceNpScoreGetClansRankingByRangeAsync(
	s32 transId, SceNpScoreClansBoardId clanBoardId,
	SceNpScoreRankNumber startSerialRank,
	vm::ptr<SceNpScoreClanIdRankData> rankArray, u32 rankArraySize,
	vm::ptr<void> reserved1, u32 reservedSize1, vm::ptr<void> reserved2,
...
{
	sceNp.todo(
		"sceNpScoreGetClansRankingByRangeAsync(transId=%d, clanBoardId=%d, "
		"startSerialRank=%d, rankArray=*0x%x, rankArraySize=%d, reserved1=*0x%x, "
		"reservedSize1=%d, reserved2=*0x%x, "
		"reservedSize2=%d, arrayNum=%d, lastSortDate=*0x%x, totalRecord=*0x%x, "
		"prio=%d, option=*0x%x)",
		transId, clanBoardId, startSerialRank, rankArray, rankArraySize,
		reserved1, reservedSize1, reserved2, reservedSize2, arrayNum,
		lastSortDate, totalRecord, prio, option);
...
error_code
sceNpScoreGetClanMemberGameData(s32 transId, SceNpScoreBoardId boardId,
	SceNpClanId clanId, vm::cptr<SceNpId> npId,
	vm::ptr<u32> totalSize, u32 recvSize,
	vm::ptr<void> data, vm::ptr<void> option)
{
	sceNp.todo(
		"sceNpScoreGetClanMemberGameData(transId=%d, boardId=%d, clanId=%d, "
		"npId=*0x%x, totalSize=*0x%x, recvSize=%d, data=*0x%x, option=*0x%x)",
...
error_code sceNpScoreGetClanMemberGameDataAsync(
	s32 transId, SceNpScoreBoardId boardId, SceNpClanId clanId,
	vm::cptr<SceNpId> npId, vm::ptr<u32> totalSize, u32 recvSize,
	vm::ptr<void> data, s32 prio, vm::ptr<void> option)
{
	sceNp.todo("sceNpScoreGetClanMemberGameDataAsync(transId=%d, boardId=%d, "
			   "clanId=%d, npId=*0x%x, totalSize=*0x%x, recvSize=%d, data=*0x%x, "
			   "prio=%d, option=*0x%x)",
		transId, boardId, clanId, npId, totalSize, recvSize, data, prio,
		option);
...
error_code sceNpScoreGetClansRankingByClanId(
	s32 transId, SceNpScoreClansBoardId clanBoardId,
	vm::cptr<SceNpClanId> clanIdArray, u32 clanIdArraySize,
	vm::ptr<SceNpScoreClanIdRankData> rankArray, u32 rankArraySize,
	vm::ptr<void> reserved1, u32 reservedSize1, vm::ptr<void> reserved2,
	u32 reservedSize2, u32 arrayNum, vm::ptr<CellRtcTick> lastSortDate,
	vm::ptr<SceNpScoreRankNumber> totalRecord, vm::ptr<void> option)
{
	sceNp.todo("sceNpScoreGetClansRankingByClanId(transId=%d, clanBoardId=%d, "
			   "clanIdArray=*0x%x, clanIdArraySize=%d, rankArray=*0x%x, "
			   "rankArraySize=%d, reserved1=*0x%x, reservedSize1=%d, "
			   "reserved2=*0x%x, reservedSize2=%d, arrayNum=%d, "
			   "lastSortDate=*0x%x, totalRecord=*0x%x, option=*0x%x)",
		transId, clanBoardId, clanIdArray, clanIdArraySize, rankArray,
		rankArraySize, reserved1, reservedSize1, reserved2, reservedSize2,
		arrayNum, lastSortDate, totalRecord, option);
	...
	if (!clanIdArray || !rankArray || !totalRecord || !lastSortDate ||
		!arrayNum)
	{
		return SCE_NP_COMMUNITY_ERROR_INSUFFICIENT_ARGUMENT;
	}

	if (reserved1 || reservedSize1 || reserved2 || reservedSize2 ||
		option) // reserved and option checks at least until fw 4.71
	{
		return SCE_NP_COMMUNITY_ERROR_INVALID_ARGUMENT;
	}
...
error_code sceNpScoreGetClansRankingByClanIdAsync(
	s32 transId, SceNpScoreClansBoardId clanBoardId,
	vm::cptr<SceNpClanId> clanIdArray, u32 clanIdArraySize,
	vm::ptr<SceNpScoreClanIdRankData> rankArray, u32 rankArraySize,
	vm::ptr<void> reserved1, u32 reservedSize1, vm::ptr<void> reserved2,
	...
	vm::ptr<SceNpScoreRankNumber> totalRecord, s32 prio, vm::ptr<void> option)
{
	sceNp.todo(
		...
		"clanIdArray=*0x%x, clanIdArraySize=%d, rankArray=*0x%x, "
		"rankArraySize=%d, reserved1=*0x%x, reservedSize1=%d, "
		"reserved2=*0x%x, reservedSize2=%d, arrayNum=%d, lastSortDate=*0x%x, "
		"totalRecord=*0x%x, prio=%d, option=*0x%x)",
		transId, clanBoardId, clanIdArray, clanIdArraySize, rankArray,
		rankArraySize, reserved1, reservedSize1, reserved2, reservedSize2,
		arrayNum, lastSortDate, totalRecord, prio, option);
...
error_code sceNpScoreGetClansMembersRankingByRange(
	s32 transId, SceNpClanId clanId, SceNpScoreBoardId boardId,
	SceNpScoreRankNumber startSerialRank,
	vm::ptr<SceNpScoreClanIdRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
	...
	vm::ptr<SceNpScoreRankNumber> totalRecord, vm::ptr<void> option)
{
	sceNp.todo("sceNpScoreGetClansMembersRankingByRange(transId=%d, clanId=%d, "
			   "boardId=%d, startSerialRank=%d, rankArray=*0x%x, "
			   "rankArraySize=%d, commentArray=*0x%x, commentArraySize=%d, "
			   "infoArray=*0x%x, infoArraySize=%d, descriptArray=*0x%x, "
			   "descriptArraySize=%d, arrayNum=%d, clanInfo=*0x%x, "
			   "lastSortDate=*0x%x, totalRecord=*0x%x, option=*0x%x)",
		transId, clanId, boardId, startSerialRank, rankArray,
		rankArraySize, commentArray, commentArraySize, infoArray,
		infoArraySize, descriptArray, descriptArraySize, arrayNum,
		clanInfo, lastSortDate, totalRecord, option);
...
error_code sceNpScoreGetClansMembersRankingByRangeAsync(
	s32 transId, SceNpClanId clanId, SceNpScoreBoardId boardId,
	SceNpScoreRankNumber startSerialRank,
	vm::ptr<SceNpScoreClanIdRankData> rankArray, u32 rankArraySize,
	vm::ptr<SceNpScoreComment> commentArray, u32 commentArraySize,
	...
	vm::ptr<SceNpScoreRankNumber> totalRecord, s32 prio, vm::ptr<void> option)
{
	sceNp.todo("sceNpScoreGetClansMembersRankingByRangeAsync(transId=%d, "
			   "clanId=%d, boardId=%d, startSerialRank=%d, rankArray=*0x%x, "
			   "rankArraySize=%d, commentArray=*0x%x, commentArraySize=%d, "
			   "infoArray=*0x%x, infoArraySize=%d, descriptArray=*0x%x, "
			   "descriptArraySize=%d, arrayNum=%d, clanInfo=*0x%x, "
			   "lastSortDate=*0x%x, totalRecord=*0x%x, prio=%d, option=*0x%x)",
		transId, clanId, boardId, startSerialRank, rankArray,
		rankArraySize, commentArray, commentArraySize, infoArray,
		infoArraySize, descriptArray, descriptArraySize, arrayNum,
		clanInfo, lastSortDate, totalRecord, prio, option);
...
error_code sceNpSignalingCreateCtx(vm::ptr<SceNpId> npId,
	vm::ptr<SceNpSignalingHandler> handler,
	vm::ptr<void> arg, vm::ptr<u32> ctx_id)
{
	sceNp.warning("sceNpSignalingCreateCtx(npId=*0x%x, handler=*0x%x, arg=*0x%x, "
				  "ctx_id=*0x%x)",
		npId, handler, arg, ctx_id);
...
error_code sceNpSignalingAddExtendedHandler(
	u32 ctx_id, vm::ptr<SceNpSignalingHandler> handler, vm::ptr<void> arg)
{
	sceNp.warning(
		"sceNpSignalingAddExtendedHandler(ctx_id=%d, handler=*0x%x, arg=*0x%x)",
		ctx_id, handler, arg);
...
{
	sceNp.todo("sceNpSignalingSetCtxOpt(ctx_id=%d, optname=%d, optval=%d)",
		ctx_id, optname, optval);
...
error_code sceNpSignalingGetCtxOpt(u32 ctx_id, s32 optname,
	vm::ptr<s32> optval)
{
	sceNp.todo("sceNpSignalingGetCtxOpt(ctx_id=%d, optname=%d, optval=*0x%x)",
		ctx_id, optname, optval);
...
error_code sceNpSignalingActivateConnection(u32 ctx_id, vm::ptr<SceNpId> npId,
	vm::ptr<u32> conn_id)
{
	sceNp.warning(
		"sceNpSignalingActivateConnection(ctx_id=%d, npId=*0x%x, conn_id=*0x%x)",
		ctx_id, npId, conn_id);
...
{
	sceNp.warning("sceNpSignalingDeactivateConnection(ctx_id=%d, conn_id=%d)",
		ctx_id, conn_id);
...
{
	sceNp.warning("sceNpSignalingTerminateConnection(ctx_id=%d, conn_id=%d)",
		ctx_id, conn_id);
...
error_code sceNpSignalingGetConnectionStatus(u32 ctx_id, u32 conn_id,
	vm::ptr<s32> conn_status,
	vm::ptr<np_in_addr> peer_addr,
	vm::ptr<np_in_port_t> peer_port)
{
	sceNp.warning("sceNpSignalingGetConnectionStatus(ctx_id=%d, conn_id=%d, "
				  "conn_status=*0x%x, peer_addr=*0x%x, peer_port=*0x%x)",
		ctx_id, conn_id, conn_status, peer_addr, peer_port);
...
error_code
sceNpSignalingGetConnectionInfo(u32 ctx_id, u32 conn_id, s32 code,
	vm::ptr<SceNpSignalingConnectionInfo> info)
{
	sceNp.warning("sceNpSignalingGetConnectionInfo(ctx_id=%d, conn_id=%d, "
				  "code=%d, info=*0x%x)",
		ctx_id, conn_id, code, info);
...
error_code sceNpSignalingGetConnectionFromNpId(u32 ctx_id,
	vm::ptr<SceNpId> npId,
	vm::ptr<u32> conn_id)
{
	sceNp.notice("sceNpSignalingGetConnectionFromNpId(ctx_id=%d, npId=*0x%x, "
				 "conn_id=*0x%x)",
		ctx_id, npId, conn_id);
...
error_code sceNpSignalingGetConnectionFromPeerAddress(u32 ctx_id,
	np_in_addr_t peer_addr,
	np_in_port_t peer_port,
	vm::ptr<u32> conn_id)
{
	sceNp.warning("sceNpSignalingGetConnectionFromPeerAddress(ctx_id=%d, "
				  "peer_addr=0x%x, peer_port=%d, conn_id=*0x%x)",
		ctx_id, peer_addr, peer_port, conn_id);
...
{
	sceNp.warning("sceNpSignalingGetLocalNetInfo(ctx_id=%d, info=*0x%x)", ctx_id,
		info);
...
error_code sceNpSignalingGetPeerNetInfo(u32 ctx_id, vm::ptr<SceNpId> npId,
	vm::ptr<u32> req_id)
{
	sceNp.todo(
		"sceNpSignalingGetPeerNetInfo(ctx_id=%d, npId=*0x%x, req_id=*0x%x)",
		ctx_id, npId, req_id);
...
{
	sceNp.todo("sceNpSignalingCancelPeerNetInfo(ctx_id=%d, req_id=%d)", ctx_id,
		req_id);
...
error_code
sceNpSignalingGetPeerNetInfoResult(u32 ctx_id, u32 req_id,
	vm::ptr<SceNpSignalingNetInfo> info)
{
	sceNp.todo(
		"sceNpSignalingGetPeerNetInfoResult(ctx_id=%d, req_id=%d, info=*0x%x)",
		ctx_id, req_id, info);
...
{
	sceNp.trace("sceNpUtilCmpNpId(id1=*0x%x(%s), id2=*0x%x(%s))", id1,
		id1 ? np::npid_to_string(*id1) : std::string(), id2,
		id2 ? np::npid_to_string(*id2) : std::string());
...
error_code sceNpUtilCmpNpIdInOrder(vm::cptr<SceNpId> id1, vm::cptr<SceNpId> id2,
	vm::ptr<s32> order)
{
	sceNp.trace("sceNpUtilCmpNpIdInOrder(id1=*0x%x, id2=*0x%x, order=*0x%x)", id1,
		id2, order);
...
error_code sceNpUtilSetPlatformType(vm::ptr<SceNpId> npId,
	SceNpPlatformType platformType)
{
	sceNp.warning("sceNpUtilSetPlatformType(npId=*0x%x, platformType=%d)", npId,
		platformType);
	...
	case SCE_NP_PLATFORM_TYPE_PS4:
		npId->unk1[1] = "ps4\0"_u32;
		break;
	case SCE_NP_PLATFORM_TYPE_VITA:
		npId->unk1[1] = "psp2"_u32;
		break;
...
{
	sceNp.todo("_Z33_sce_np_sysutil_send_empty_packetiPN16sysutil_"
			   "cxmlutil11FixedMemoryEPKcS3_()");
	return CELL_OK;
}
...
{
	sceNp.todo("_Z40_sce_np_sysutil_recv_packet_fixedmem_subiPN16sysutil_"
			   "cxmlutil11FixedMemoryERN4cxml8DocumentERNS2_7ElementE()");
	return CELL_OK;
}
...
{
	sceNp.todo("_Z32_sce_np_sysutil_cxml_prepare_docPN16sysutil_"
