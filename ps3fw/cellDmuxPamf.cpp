...
bool dmux_pamf_base::process_next_pack()
{
	...
	case state::initial:
	{
		...
		const u8 pack_stuffing_length = read_from_ptr<bf_t<u8, 0, 3>>(pack, PACK_STUFFING_LENGTH_OFFSET);
		...
		if (read_from_ptr<bf_t<be_t<u32>, 8, 24>>(current_pes_packet) != PACKET_START_CODE_PREFIX)
		{
...
{
	// This is repeated a lot in this function, in my opinion using a define here makes it more readable
#define RETURN_ON_CPU_FLAG_AGAIN()\
	...
	case dmux_pamf_state::sending_fatal_err: ; // Handled below
	}

	for (;;)
	{
		if (savestate == dmux_pamf_state::sending_fatal_err)
		{
		...
		case DmuxPamfEventType::au_found:
		{
			...
			{
				es->au_specific_info[0] = read_from_ptr<bf_t<u8, 4, 4>>(event.au_found.stream_header_buf);
				es->au_specific_info[1] = read_from_ptr<bf_t<u8, 0, 4>>(event.au_found.stream_header_buf);
	...
	const DmuxCb<DmuxNotifyProgEndCode>& notify_prog_end_code, const DmuxCb<DmuxNotifyFatalErr>& notify_fatal_err, vm::bptr<DmuxPamfContext>& handle)
{
	...
	const auto _this = vm::ptr<DmuxPamfContext>::make(rx::alignUp(+res.memAddr.addr(), 0x80));
	...
	const DmuxCb<DmuxEsNotifyFlushDone>& notify_flush_done, vm::bptr<DmuxPamfElementaryStream>& es)
{
	...
	const auto _es = vm::bptr<DmuxPamfElementaryStream>::make(rx::alignUp(mem_addr.addr(), 0x10));

	const auto au_queue_buffer = vm::bptr<u8>::make(rx::alignUp(_es.addr() + static_cast<u32>(sizeof(DmuxPamfElementaryStream)), 0x80));
