...
void fmt_class_string<lv2_socket_family>::format(std::string &out, u64 arg) {
  format_enum(out, arg, [](auto value) {
    switch (value) {
    case SYS_NET_AF_UNSPEC:
      return "UNSPEC";
    case SYS_NET_AF_LOCAL:
      return "LOCAL";
    case SYS_NET_AF_INET:
      return "INET";
    case SYS_NET_AF_INET6:
      return "INET6";
    }
...
void fmt_class_string<lv2_ip_protocol>::format(std::string &out, u64 arg) {
  format_enum(out, arg, [](auto value) {
    ...
    case SYS_NET_IPPROTO_IP:
      return "IPPROTO_IP";
    case SYS_NET_IPPROTO_ICMP:
      return "IPPROTO_ICMP";
    case SYS_NET_IPPROTO_IGMP:
      return "IPPROTO_IGMP";
    case SYS_NET_IPPROTO_TCP:
      return "IPPROTO_TCP";
    case SYS_NET_IPPROTO_UDP:
      return "IPPROTO_UDP";
    case SYS_NET_IPPROTO_ICMPV6:
...
void fmt_class_string<lv2_tcp_option>::format(std::string &out, u64 arg) {
  format_enum(out, arg, [](auto value) {
    ...
    case SYS_NET_TCP_MAXSEG:
      return "TCP_MAXSEG";
    case SYS_NET_TCP_MSS_TO_ADVERTISE:
      return "TCP_MSS_TO_ADVERTISE";
    }
...
lv2_socket::lv2_socket(utils::serial &ar, lv2_socket_type _type)
    : family(ar), type(_type), protocol(ar), so_nbio(ar), so_error(ar),
      so_tcp_maxseg(ar)
#ifdef _WIN32
      ,
...
std::function<void(void *)> lv2_socket::load(utils::serial &ar) {
  ...
  case SYS_NET_SOCK_RAW:
    sock_lv2 = make_shared<lv2_socket_raw>(ar, type);
    break;
  case SYS_NET_SOCK_DGRAM_P2P:
    sock_lv2 = make_shared<lv2_socket_p2p>(ar, type);
    break;
  case SYS_NET_SOCK_STREAM_P2P:
    sock_lv2 = make_shared<lv2_socket_p2ps>(ar, type);
    break;
  }

  if (std::memcmp(&sock_lv2->last_bound_addr, std::array<u8, 16>{}.data(),
                  16)) {
...
void lv2_socket::save(utils::serial &ar, bool save_only_this_class) {
  ...
  case SYS_NET_SOCK_RAW:
    static_cast<lv2_socket_raw *>(this)->save(ar);
    break;
  case SYS_NET_SOCK_DGRAM_P2P:
    static_cast<lv2_socket_p2p *>(this)->save(ar);
    break;
  case SYS_NET_SOCK_STREAM_P2P:
    static_cast<lv2_socket_p2ps *>(this)->save(ar);
    break;
  }
}
...
void sys_net_dump_data(std::string_view desc, const u8 *data, s32 len,
                       const void *addr) {
  const sys_net_sockaddr_in_p2p *p2p_addr =
      reinterpret_cast<const sys_net_sockaddr_in_p2p *>(addr);

  if (p2p_addr)
    sys_net_dump.trace("%s(%s:%d:%d): %s", desc,
                       np::ip_to_string(std::bit_cast<u32>(p2p_addr->sin_addr)),
                       p2p_addr->sin_port, p2p_addr->sin_vport,
                       fmt::buf_to_hexstring(data, len));
  else
    sys_net_dump.trace("%s: %s", desc, fmt::buf_to_hexstring(data, len));
}

error_code sys_net_bnet_accept(ppu_thread &ppu, s32 s,
                               vm::ptr<sys_net_sockaddr> addr,
                               vm::ptr<u32> paddrlen) {
  ppu.state += cpu_flag::wait;

  sys_net.warning("sys_net_bnet_accept(s=%d, addr=*0x%x, paddrlen=*0x%x)", s,
                  addr, paddrlen);

  if (addr.operator bool() != paddrlen.operator bool() ||
      (paddrlen && *paddrlen < addr.size())) {
  ...
  const auto sock = idm::check<lv2_socket>(
      s, [&, notify = lv2_obj::notify_all_t()](lv2_socket &sock) {
...
error_code sys_net_bnet_bind(ppu_thread &ppu, s32 s,
                             vm::cptr<sys_net_sockaddr> addr, u32 addrlen) {
  ppu.state += cpu_flag::wait;

  sys_net.warning("sys_net_bnet_bind(s=%d, addr=*0x%x, addrlen=%u)", s, addr,
                  addrlen);
  ...
  if (sn_addr.sa_family != SYS_NET_AF_INET &&
      sn_addr.sa_family != SYS_NET_AF_UNSPEC) {
    sys_net.error("sys_net_bnet_bind: unsupported sa_family (%d)",
                  sn_addr.sa_family);
    return -SYS_NET_EAFNOSUPPORT;
  }

  const auto sock = idm::check<lv2_socket>(
      s, [&, notify = lv2_obj::notify_all_t()](lv2_socket &sock) -> s32 {
...
error_code sys_net_bnet_connect(ppu_thread &ppu, s32 s,
                                vm::ptr<sys_net_sockaddr> addr, u32 addrlen) {
  ppu.state += cpu_flag::wait;

  sys_net.warning("sys_net_bnet_connect(s=%d, addr=*0x%x, addrlen=%u)", s, addr,
                  addrlen);
  ...
  if (addr->sa_family != SYS_NET_AF_INET) {
    sys_net.error("sys_net_bnet_connect(s=%d): unsupported sa_family (%d)", s,
                  addr->sa_family);
    return -SYS_NET_EAFNOSUPPORT;
  }
...
error_code sys_net_bnet_getpeername(ppu_thread &ppu, s32 s,
                                    vm::ptr<sys_net_sockaddr> addr,
                                    vm::ptr<u32> paddrlen) {
  ppu.state += cpu_flag::wait;

  sys_net.warning("sys_net_bnet_getpeername(s=%d, addr=*0x%x, paddrlen=*0x%x)",
                  s, addr, paddrlen);
...
error_code sys_net_bnet_getsockname(ppu_thread &ppu, s32 s,
                                    vm::ptr<sys_net_sockaddr> addr,
                                    vm::ptr<u32> paddrlen) {
  ppu.state += cpu_flag::wait;

  sys_net.warning("sys_net_bnet_getsockname(s=%d, addr=*0x%x, paddrlen=*0x%x)",
                  s, addr, paddrlen);
...
error_code sys_net_bnet_getsockopt(ppu_thread &ppu, s32 s, s32 level,
                                   s32 optname, vm::ptr<void> optval,
                                   vm::ptr<u32> optlen) {
  ppu.state += cpu_flag::wait;

  switch (level) {
  case SYS_NET_SOL_SOCKET:
    sys_net.warning("sys_net_bnet_getsockopt(s=%d, level=SYS_NET_SOL_SOCKET, "
                    "optname=%s, optval=*0x%x, optlen=%u)",
                    s, static_cast<lv2_socket_option>(optname), optval, optlen);
    break;
  case SYS_NET_IPPROTO_TCP:
    sys_net.warning("sys_net_bnet_getsockopt(s=%d, level=SYS_NET_IPPROTO_TCP, "
                    "optname=%s, optval=*0x%x, optlen=%u)",
                    s, static_cast<lv2_tcp_option>(optname), optval, optlen);
    break;
  case SYS_NET_IPPROTO_IP:
    sys_net.warning("sys_net_bnet_getsockopt(s=%d, level=SYS_NET_IPPROTO_IP, "
                    "optname=%s, optval=*0x%x, optlen=%u)",
                    s, static_cast<lv2_ip_option>(optname), optval, optlen);
    break;
  default:
    sys_net.warning("sys_net_bnet_getsockopt(s=%d, level=0x%x, optname=0x%x, "
                    "optval=*0x%x, optlen=%u)",
  ...
  const auto sock = idm::check<lv2_socket>(
      s, [&, notify = lv2_obj::notify_all_t()](lv2_socket &sock) -> s32 {
        ...
        const auto &[res, out_val, out_len] =
            sock.getsockopt(level, optname, *optlen);
...
error_code sys_net_bnet_recvfrom(ppu_thread &ppu, s32 s, vm::ptr<void> buf,
                                 u32 len, s32 flags,
                                 vm::ptr<sys_net_sockaddr> addr,
                                 vm::ptr<u32> paddrlen) {
  ppu.state += cpu_flag::wait;

  sys_net.trace("sys_net_bnet_recvfrom(s=%d, buf=*0x%x, len=%u, flags=0x%x, "
                "addr=*0x%x, paddrlen=*0x%x)",
                s, buf, len, flags, addr, paddrlen);
  ...
  if (flags & ~(SYS_NET_MSG_PEEK | SYS_NET_MSG_DONTWAIT | SYS_NET_MSG_WAITALL |
                SYS_NET_MSG_USECRYPTO | SYS_NET_MSG_USESIGNATURE)) {
    fmt::throw_exception("sys_net_bnet_recvfrom(s=%d): unknown flags (0x%x)",
                         flags);
  }
...
error_code sys_net_bnet_recvmsg(ppu_thread &ppu, s32 s,
                                vm::ptr<sys_net_msghdr> msg, s32 flags) {
  ppu.state += cpu_flag::wait;

  sys_net.todo("sys_net_bnet_recvmsg(s=%d, msg=*0x%x, flags=0x%x)", s, msg,
               flags);
  return CELL_OK;
}

error_code sys_net_bnet_sendmsg(ppu_thread &ppu, s32 s,
                                vm::cptr<sys_net_msghdr> msg, s32 flags) {
  ppu.state += cpu_flag::wait;

  sys_net.warning("sys_net_bnet_sendmsg(s=%d, msg=*0x%x, flags=0x%x)", s, msg,
                  flags);

  if (flags & ~(SYS_NET_MSG_DONTWAIT | SYS_NET_MSG_WAITALL |
                SYS_NET_MSG_USECRYPTO | SYS_NET_MSG_USESIGNATURE)) {
    fmt::throw_exception("sys_net_bnet_sendmsg(s=%d): unknown flags (0x%x)",
                         flags);
  }
...
error_code sys_net_bnet_sendto(ppu_thread &ppu, s32 s, vm::cptr<void> buf,
                               u32 len, s32 flags,
                               vm::cptr<sys_net_sockaddr> addr, u32 addrlen) {
  ppu.state += cpu_flag::wait;

  sys_net.trace("sys_net_bnet_sendto(s=%d, buf=*0x%x, len=%u, flags=0x%x, "
                "addr=*0x%x, addrlen=%u)",
                s, buf, len, flags, addr, addrlen);

  if (flags & ~(SYS_NET_MSG_DONTWAIT | SYS_NET_MSG_WAITALL |
                SYS_NET_MSG_USECRYPTO | SYS_NET_MSG_USESIGNATURE)) {
    fmt::throw_exception("sys_net_bnet_sendto(s=%d): unknown flags (0x%x)",
                         flags);
  }
  ...
  if (addr && addr->sa_family != SYS_NET_AF_INET) {
    sys_net.error("sys_net_bnet_sendto(s=%d): unsupported sa_family (%d)", s,
                  addr->sa_family);
    return -SYS_NET_EAFNOSUPPORT;
  }

  sys_net_dump_data("sendto", static_cast<const u8 *>(buf.get_ptr()), len,
                    addr ? addr.get_ptr() : nullptr);

  const std::optional<sys_net_sockaddr> sn_addr =
      addr ? std::optional<sys_net_sockaddr>(*addr) : std::nullopt;
  const std::vector<u8> buf_copy(vm::_ptr<const char>(buf.addr()),
                                 vm::_ptr<const char>(buf.addr()) + len);
  s32 result{};
      ...
      s, [&, notify = lv2_obj::notify_all_t()](lv2_socket &sock) {
        ...
        sock.poll_queue(idm::get_unlocked<named_thread<ppu_thread>>(ppu.id),
                        ...
                        [&](rx::EnumBitSet<lv2_socket::poll_t> events) -> bool {
                          ...
                          if (sock.so_sendtimeo &&
                              get_guest_system_time() - ppu.start_time >
                                  sock.so_sendtimeo) {
                            result = -SYS_NET_EWOULDBLOCK;
                            lv2_obj::awake(&ppu);
                            return true;
...
error_code sys_net_bnet_setsockopt(ppu_thread &ppu, s32 s, s32 level,
                                   s32 optname, vm::cptr<void> optval,
                                   u32 optlen) {
  ppu.state += cpu_flag::wait;

  switch (level) {
  case SYS_NET_SOL_SOCKET:
    sys_net.warning("sys_net_bnet_setsockopt(s=%d, level=SYS_NET_SOL_SOCKET, "
                    "optname=%s, optval=*0x%x, optlen=%u)",
                    s, static_cast<lv2_socket_option>(optname), optval, optlen);
    break;
  case SYS_NET_IPPROTO_TCP:
    sys_net.warning("sys_net_bnet_setsockopt(s=%d, level=SYS_NET_IPPROTO_TCP, "
                    "optname=%s, optval=*0x%x, optlen=%u)",
                    s, static_cast<lv2_tcp_option>(optname), optval, optlen);
    break;
  case SYS_NET_IPPROTO_IP:
    sys_net.warning("sys_net_bnet_setsockopt(s=%d, level=SYS_NET_IPPROTO_IP, "
                    "optname=%s, optval=*0x%x, optlen=%u)",
                    s, static_cast<lv2_ip_option>(optname), optval, optlen);
    break;
  default:
    sys_net.warning("sys_net_bnet_setsockopt(s=%d, level=0x%x, optname=0x%x, "
                    "optval=*0x%x, optlen=%u)",
                    s, level, optname, optval, optlen);
    break;
  }
  ...
  switch (optlen) {
  case 1:
    sys_net.warning("optval: 0x%02X",
                    *static_cast<const u8 *>(optval.get_ptr()));
    break;
  case 2:
    sys_net.warning("optval: 0x%04X",
                    *static_cast<const be_t<u16> *>(optval.get_ptr()));
    break;
  case 4:
    sys_net.warning("optval: 0x%08X",
                    *static_cast<const be_t<u32> *>(optval.get_ptr()));
    break;
  case 8:
    sys_net.warning("optval: 0x%016X",
                    *static_cast<const be_t<u64> *>(optval.get_ptr()));
    break;
  }
  ...
  std::vector<u8> optval_copy(vm::_ptr<u8>(optval.addr()),
                              vm::_ptr<u8>(optval.addr() + optlen));
...
error_code sys_net_bnet_socket(ppu_thread &ppu, lv2_socket_family family,
                               lv2_socket_type type, lv2_ip_protocol protocol) {
  ppu.state += cpu_flag::wait;

  sys_net.warning("sys_net_bnet_socket(family=%s, type=%s, protocol=%s)",
  ...
  if (type != SYS_NET_SOCK_STREAM && type != SYS_NET_SOCK_DGRAM &&
      type != SYS_NET_SOCK_RAW && type != SYS_NET_SOCK_DGRAM_P2P &&
      type != SYS_NET_SOCK_STREAM_P2P) {
  ...
  case SYS_NET_SOCK_RAW:
    sock_lv2 = make_shared<lv2_socket_raw>(family, type, protocol);
    break;
  case SYS_NET_SOCK_DGRAM_P2P:
    sock_lv2 = make_shared<lv2_socket_p2p>(family, type, protocol);
    break;
  case SYS_NET_SOCK_STREAM_P2P:
    sock_lv2 = make_shared<lv2_socket_p2ps>(family, type, protocol);
    break;
  }
...
error_code sys_net_bnet_poll(ppu_thread &ppu, vm::ptr<sys_net_pollfd> fds,
                             s32 nfds, s32 ms) {
  ppu.state += cpu_flag::wait;
...
error_code sys_net_bnet_select(ppu_thread &ppu, s32 nfds,
                               vm::ptr<sys_net_fd_set> readfds,
                               vm::ptr<sys_net_fd_set> writefds,
                               vm::ptr<sys_net_fd_set> exceptfds,
                               vm::ptr<sys_net_timeval> _timeout) {
  ppu.state += cpu_flag::wait;

  sys_net.trace("sys_net_bnet_select(nfds=%d, readfds=*0x%x, writefds=*0x%x, "
                "exceptfds=*0x%x, timeout=*0x%x(%d:%d))",
                nfds, readfds, writefds, exceptfds, _timeout,
                _timeout ? _timeout->tv_sec.value() : 0,
                _timeout ? _timeout->tv_usec.value() : 0);
  ...
  u64 timeout =
      !_timeout ? 0 : _timeout->tv_sec * 1000000ull + _timeout->tv_usec;
  ...
  if (nfds > 0 && nfds <= 1024) {
    ...
    for (s32 i = 0; i < nfds; i++) {
      ...
      if (auto sock = idm::check_unlocked<lv2_socket>(
              (lv2_socket::id_base & -1024) + i)) {
        auto [read_set, write_set, except_set] =
            sock->select(selected, _fds[i]);
             ...
             &ppu](rx::EnumBitSet<lv2_socket::poll_t> events) {
              if (events & selected) {
                if (selected & lv2_socket::poll_t::read &&
                    events &
                        (lv2_socket::poll_t::read + lv2_socket::poll_t::error))
                  rread.set(i);
                if (selected & lv2_socket::poll_t::write &&
                    events &
                        (lv2_socket::poll_t::write + lv2_socket::poll_t::error))
                  rwrite.set(i);
...
error_code _sys_net_read_dump(ppu_thread &ppu, s32 id, vm::ptr<void> buf,
                              s32 len, vm::ptr<s32> pflags) {
  ppu.state += cpu_flag::wait;

  sys_net.todo("_sys_net_read_dump(id=0x%x, buf=*0x%x, len=%d, pflags=*0x%x)",
               id, buf, len, pflags);
  return CELL_OK;
}
...
error_code _sys_net_write_dump(ppu_thread &ppu, s32 id, vm::cptr<void> buf,
                               s32 len, u32 unknown) {
  ppu.state += cpu_flag::wait;

  sys_net.todo("_sys_net_write_dump(id=0x%x, buf=*0x%x, len=%d, unk=0x%x)", id,
               buf, len, unknown);
  return CELL_OK;
}
...
error_code lv2_socket::abort_socket(s32 flags) {
  decltype(queue) qcopy;
  {
    ...
    if (queue.empty()) {
      ...
      // TODO: Abort the subsequent function called on this socket (need to
      // investigate correct behaviour)
      return CELL_OK;
    }
  ...
  for (auto &[ppu, _] : qcopy) {
    ...
    sys_net.warning("lv2_socket::abort_socket(): waking up \"%s\": (func: %s, "
                    "r3=0x%x, r4=0x%x, r5=0x%x, r6=0x%x)",
                    ppu->get_name(), ppu->current_function, ppu->gpr[3],
...
error_code sys_net_abort(ppu_thread &ppu, s32 type, u64 arg, s32 flags) {
  ...
  sys_net.warning("sys_net_abort(type=%d, arg=0x%x, flags=0x%x)", type, arg,
                  flags);
  ...
  case all: {
    ...
    idm::select<lv2_socket>(
        [&](u32 id, lv2_socket &) { sockets.emplace_back(id); });
...
error_code sys_net_infoctl(ppu_thread &ppu, s32 cmd, vm::ptr<void> arg) {
  ...
  case 9: {
    ...
    std::memcpy(
        vm::static_ptr_cast<net_infoctl_cmd_9_t>(arg)->server_name.get_ptr(),
        name.data(), name.size());
    break;
  }
  default:
...
error_code sys_net_control(ppu_thread &ppu, u32 arg1, s32 arg2,
                           vm::ptr<void> arg3, s32 arg4) {
  ppu.state += cpu_flag::wait;
...
error_code sys_net_bnet_sysctl(ppu_thread &ppu, u32 arg1, u32 arg2, u32 arg3,
                               vm::ptr<void> arg4, u32 arg5, u32 arg6) {
  ppu.state += cpu_flag::wait;

  sys_net.todo("sys_net_bnet_sysctl(0x%x, 0x%x, 0x%x, *0x%x, 0x%x, 0x%x)", arg1,
               arg2, arg3, arg4, arg5, arg6);
  return CELL_OK;
}

error_code sys_net_eurus_post_command(ppu_thread &ppu, s32 arg1, u32 arg2,
                                      u32 arg3) {
  ppu.state += cpu_flag::wait;
