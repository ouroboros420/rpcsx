...
void fmt_class_string<libusb_transfer>::format(std::string &out, u64 arg) {
  ...
  const int data_start = transfer.type == LIBUSB_TRANSFER_TYPE_CONTROL
                             ? LIBUSB_CONTROL_SETUP_SIZE
                             : 0;
  fmt::append(out, "TR[r:%d][sz:%d] => %s", +transfer.status,
              transfer.actual_length,
              fmt::buf_to_hexstring(&transfer.buffer[data_start],
                                    transfer.actual_length));
}
...
struct usb_allow_list_entry {
  ...
  auto operator<(const usb_allow_list_entry &r) const {
    return std::tuple(id_vendor, id_product_min, id_product_max, device_name,
                      max_device_count, make_instance) <
           std::tuple(r.id_vendor, r.id_product_min, r.id_product_max,
                      device_name, max_device_count, make_instance);
  }
};
...
public:
  ...
  void save(utils::serial &ar) { ar(u8{is_init.load()}); }
  ...
  bool add_ldd(std::string_view product, u16 id_vendor, u16 id_product_min,
               u16 id_product_max);
  ...
  std::pair<u32, UsbDeviceIsoRequest>
  get_isochronous_transfer_status(u32 transfer_id);
  ...
  std::map<u32, std::pair<UsbInternalDevice, std::shared_ptr<usb_device>>>
      handled_devices;
  std::map<u8, std::pair<input::product_type, std::shared_ptr<usb_device>>>
      pad_to_usb;
...
private:
  // Lock free functions for internal use(ie make sure to lock before using
  // those)
...
private:
  ...
  std::unordered_map<std::string, UsbLdd, fmt::string_hash, std::equal_to<>>
      ldds;

  const std::vector<usb_allow_list_entry> device_allow_list{
      // Portals
      {0x1430, 0x0150, 0x0150, "Skylanders Portal",
       &usb_device_skylander::get_num_emu_devices,
       &usb_device_skylander::make_instance},
      {0x0E6F, 0x0129, 0x0129, "Disney Infinity Base",
       &usb_device_infinity::get_num_emu_devices,
       &usb_device_infinity::make_instance},
      ...
      // {0x1415, 0x0020, 0x0020, "SingStar Microphone Wireless", nullptr,
      // nullptr}, // TODO: verifiy
      // {0x12ba, 0x00f0, 0x00f0, "Bandfuse USB Guitar Adapter", nullptr,
      // nullptr},
      ...
      {0x12BA, 0x074B, 0x074B, "Guitar Hero Live Guitar",
       &usb_device_ghltar::get_num_emu_devices,
       &usb_device_ghltar::make_instance},

      {0x12BA, 0x0140, 0x0140, "DJ Hero Turntable",
       &usb_device_turntable::get_num_emu_devices,
       &usb_device_turntable::make_instance},
      ...
      {0x1BAD, 0x3430, 0x343F, "Harmonix Button Guitar - Wii", nullptr,
       nullptr},
      ...
      {0x046D, 0xC283, 0xC29B, "lgFF_c283_c29b",
       &usb_device_logitech_g27::get_num_emu_devices,
       &usb_device_logitech_g27::make_instance},
#else
      ...
      {0x044F, 0xb655, 0xb655, "Thrustmaster FGT Rumble 3-in-1", nullptr,
       nullptr},
      ...
      {0x044F, 0xB660, 0xB660, "Thrustmaster T500 RS Gear Shift", nullptr,
       nullptr},
      ...
      // Buzz controllers
      {0x054C, 0x1000, 0x1040, "buzzer0", &usb_device_buzz::get_num_emu_devices,
       &usb_device_buzz::make_instance},
      ...
      {0x0B9A, 0x0900, 0x0910, "PS3A-USJ",
       &usb_device_usio::get_num_emu_devices, &usb_device_usio::make_instance},

      // Densha de GO! controller
      {0x0AE4, 0x0004, 0x0004, "Densha de GO! Type 2 Controller", nullptr,
       nullptr},
  ...
  std::unordered_map<uint64_t, std::shared_ptr<usb_device>>
...
static void LIBUSB_CALL log_cb(libusb_context * /*ctx*/,
...
void usb_handler_thread::perform_scan() {
  ...
  if (ndev < 0) {
    sys_usbd.error("Failed to get device list: %s",
  ...
  for (auto index = 0; index < ndev; index++) {
    ...
    if (int res = libusb_get_device_descriptor(dev, &desc); res < 0) {
      sys_usbd.error("Failed to get device descriptor: %s",
    ...
    const u64 usb_id = (static_cast<uint64_t>(desc.idVendor) << 48) |
                       (static_cast<uint64_t>(desc.idProduct) << 32) |
                       (static_cast<uint64_t>(port) << 8) | address;
...
#ifdef __APPLE__
        // On macOS, libusb cannot claim HID interfaces, so passing through a
        // real controller that also has an emulated implementation yields a
        // non-functional device and silently overrides the user's
        // emulated-device setting (the emulated setup below only fills slots
        // not already passed through). Prefer the emulated implementation
        // whenever the user has enabled it. (e.g. DJ Hero Turntable)
        if (entry.make_instance && entry.max_device_count &&
            entry.max_device_count() > 0) {
          continue;
        }
#endif
        sys_usbd.success("Found device: %s",
                         std::basic_string(entry.device_name));
        libusb_ref_device(dev);
        std::shared_ptr<usb_device_passthrough> usb_dev =
            std::make_shared<usb_device_passthrough>(dev, desc,
                                                     get_new_location());
    ...
    if (desc.idVendor == 0x1209 && desc.idProduct == 0x2882) {
      ...
      // Send the device a specific control transfer so that it jumps to a RPCS3
      // compatible mode
      ...
      if (libusb_open(dev, &lusb_handle) == LIBUSB_SUCCESS) {
        ...
        libusb_control_transfer(lusb_handle,
                                ...
                                0x01, 0x03f2, 2, nullptr, 0, 5000);
        libusb_close(lusb_handle);
      } else {
        sys_usbd.error("Unable to open Santroller device, make sure Santroller "
                       "isn't open in the background.");
      }
    }
  }

  for (auto it = usb_passthrough_devices.begin();
       it != usb_passthrough_devices.end();) {
    auto &dev = *it;
...
usb_handler_thread::usb_handler_thread() {
  ...
  log_lv_opt.value.ival =
      LIBUSB_LOG_LEVEL_WARNING; // You can also set the LIBUSB_DEBUG env
                                // variable instead
  ...
  std::vector<libusb_init_option> options = {std::move(log_lv_opt),
                                             std::move(log_cb_opt)};

  if (int res = libusb_init_context(&ctx, options.data(),
  ...
  if (libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
    if (int res = libusb_hotplug_register_callback(
            ...
            static_cast<libusb_hotplug_callback_fn>(hotplug_callback), nullptr,
            &callback_handle);
        res < 0) {
      sys_usbd.error("Failed to initialize sys_usbd hotplug: %s",
                     libusb_error_name(res));
    } else {
  ...
  if (g_cfg.io.ghltar == ghltar_handler::one_controller ||
      g_cfg.io.ghltar == ghltar_handler::two_controllers) {
  ...
  if (g_cfg.io.turntable == turntable_handler::one_controller ||
      g_cfg.io.turntable == turntable_handler::two_controllers) {
  ...
  if (g_cfg.io.buzz == buzz_handler::one_controller ||
      g_cfg.io.buzz == buzz_handler::two_controllers) {
    if (!g_cfg_buzz.load()) {
  ...
  // Set up emulated devices for any devices that are not already being passed
  // through
  ...
  for (const auto &dev : usb_devices) {
    for (const auto &entry : device_allow_list) {
      ...
      if (entry.max_device_count != nullptr &&
          (idVendor == entry.id_vendor && idProduct >= entry.id_product_min &&
           idProduct <= entry.id_product_max)) {
  ...
  for (const auto &entry : device_allow_list) {
    if (entry.max_device_count && entry.make_instance) {
      ...
      for (int i = count; i < entry.max_device_count(); i++) {
        sys_usbd.success("Emulating device: %s (%d)",
                         std::basic_string(entry.device_name), i + 1);
  ...
  case microphone_handler::standard:
    usb_devices.push_back(std::make_shared<usb_device_mic>(
        0, get_new_location(), MicType::Logitech));
    break;
  case microphone_handler::real_singstar:
  case microphone_handler::singstar:
    usb_devices.push_back(std::make_shared<usb_device_mic>(
        0, get_new_location(), MicType::SingStar));
    break;
  case microphone_handler::rocksmith:
    usb_devices.push_back(std::make_shared<usb_device_mic>(
        0, get_new_location(), MicType::Rocksmith));
    break;
  }

  for (int i = 0; i < 8; i++) // Add VFS USB mass storage devices (/dev_usbXXX)
                              // to the USB device list
  {
    const auto usb_info =
        g_cfg_vfs.get_device(g_cfg_vfs.dev_usb, fmt::format("/dev_usb%03d", i));
    if (fs::is_dir(usb_info.path))
      usb_devices.push_back(
          std::make_shared<usb_device_vfs>(usb_info, get_new_location()));
  }
  ...
  for (usz index = 0; index < std::min(max_midi_devices, devices_list.size());
    ...
    const midi_device device =
        midi_device::from_string(::at32(devices_list, index));
    if (device.name.empty())
      continue;

    sys_usbd.notice("Adding Emulated Midi Pro Adapter (type=%s, name=%s)",
                    device.type, device.name);

    switch (device.type) {
    case midi_device_type::guitar:
      usb_devices.push_back(std::make_shared<usb_device_rb3_midi_guitar>(
          get_new_location(), device.name, false));
      break;
    case midi_device_type::guitar_22fret:
      usb_devices.push_back(std::make_shared<usb_device_rb3_midi_guitar>(
          get_new_location(), device.name, true));
      break;
    case midi_device_type::keyboard:
      usb_devices.push_back(std::make_shared<usb_device_rb3_midi_keyboard>(
          get_new_location(), device.name));
      break;
    case midi_device_type::drums:
      ...
      usb_devices.push_back(std::make_shared<usb_device_rb3_midi_drums>(
          get_new_location(), device.name));
...
void usb_handler_thread::operator()() {
  ...
  while (ctx && thread_ctrl::state() != thread_state::aborting) {
    ...
    if (now > usb_hotplug_timeout) {
      // If we did the hotplug scan each cycle the game performance was
      // significantly degraded, so we only perform this scan every 4 seconds.
      // On systems where hotplug is native, we wait a little bit for devices to
    ...
    if (libusbd_active && !fake_transfers.empty()) {
      ...
      for (auto it = fake_transfers.begin(); it != fake_transfers.end();) {
        ...
        it = fake_transfers.erase(
            it); // if we've processed this, then we erase this entry (replacing
                 // the iterator with the new reference)
      }
    }
...
void usb_handler_thread::transfer_complete(struct libusb_transfer *transfer) {
  ...
  case LIBUSB_TRANSFER_COMPLETED:
    usbd_transfer->result = HC_CC_NOERR;
    break;
  case LIBUSB_TRANSFER_TIMED_OUT:
    usbd_transfer->result = EHCI_CC_XACT;
    break;
  case LIBUSB_TRANSFER_OVERFLOW:
    usbd_transfer->result = EHCI_CC_BABBLE;
    break;
  ...
  for (s32 index = 0; index < transfer->num_iso_packets; index++) {
    ...
    usbd_transfer->iso_request.packets[index] =
        ((iso_status & 0xF) << 12 |
         (transfer->iso_packet_desc[index].actual_length & 0xFFF));
  }

  if (transfer->type == LIBUSB_TRANSFER_TYPE_CONTROL &&
      usbd_transfer->control_destbuf) {
    memcpy(usbd_transfer->control_destbuf,
  ...
  sys_usbd.trace("Transfer complete(0x%x): %s", usbd_transfer->transfer_id,
                 *transfer);
}

bool usb_handler_thread::add_ldd(std::string_view product, u16 id_vendor,
                                 u16 id_product_min, u16 id_product_max) {
  if (ldds.try_emplace(std::string(product),
                       UsbLdd{id_vendor, id_product_min, id_product_max})
          .second) {
    for (const auto &dev : usb_devices) {
      if (dev->assigned_number)
        continue;

      if (dev->device._device.idVendor == id_vendor &&
...
bool usb_handler_thread::remove_ldd(std::string_view product) {
  if (const auto iterator = ldds.find(product); iterator != ldds.end()) {
    for (const auto &dev : usb_devices) {
      ...
      if (dev->device._device.idVendor == iterator->second.id_vendor &&
          dev->device._device.idProduct >= iterator->second.id_product_min &&
          dev->device._device.idProduct <= iterator->second.id_product_max) {
...
u32 usb_handler_thread::open_pipe(u32 device_handle, u8 endpoint) {
  open_pipes.emplace(pipe_counter,
                     UsbPipe{handled_devices[device_handle].second, endpoint});
  return pipe_counter++;
}
...
bool usb_handler_thread::get_event(vm::ptr<u64> &arg1, vm::ptr<u64> &arg2,
                                   vm::ptr<u64> &arg3) {
  if (!usbd_events.empty()) {
    ...
    *arg3 = std::get<2>(usb_event);
    usbd_events.pop();
    sys_usbd.trace("Received event: arg1=0x%x arg2=0x%x arg3=0x%x", *arg1,
                   *arg2, *arg3);
    return true;
  }
...
void usb_handler_thread::add_event(u64 arg1, u64 arg2, u64 arg3) {
  ...
  if (const auto cpu = lv2_obj::schedule<ppu_thread>(sq, SYS_SYNC_PRIORITY)) {
    sys_usbd.trace("Sending event(queue): arg1=0x%x arg2=0x%x arg3=0x%x", arg1,
                   arg2, arg3);
    cpu->gpr[4] = arg1;
    cpu->gpr[5] = arg2;
    cpu->gpr[6] = arg3;
    ...
    sys_usbd.trace("Sending event: arg1=0x%x arg2=0x%x arg3=0x%x", arg1, arg2,
                   arg3);
...
std::pair<u32, UsbDeviceIsoRequest>
usb_handler_thread::get_isochronous_transfer_status(u32 transfer_id) {
  std::lock_guard lock_tf(mutex_transfers);
...
void usb_handler_thread::connect_usb_device(std::shared_ptr<usb_device> dev,
                                            bool update_usb_devices) {
  if (update_usb_devices)
    usb_devices.push_back(dev);

  for (const auto &[name, ldd] : ldds) {
    if (dev->device._device.idVendor == ldd.id_vendor &&
        dev->device._device.idProduct >= ldd.id_product_min &&
        dev->device._device.idProduct <= ldd.id_product_max) {
      if (!dev->open_device()) {
        sys_usbd.error(
            "Failed to open USB device(VID=0x%04x, PID=0x%04x) for LDD <%s>",
            dev->device._device.idVendor, dev->device._device.idProduct, name);
        disconnect_usb_device(dev);
        return;
      }

      dev->read_descriptors();
      dev->assigned_number = dev_counter++; // assign current dev_counter, and
                                            // atomically increment0
      handled_devices.emplace(
          dev->assigned_number,
          std::pair(UsbInternalDevice{0x00, narrow<u8>(dev->assigned_number),
                                      0x02, 0x40},
                    dev));
      send_message(SYS_USBD_ATTACH, dev->assigned_number);
      sys_usbd.success("USB device(VID=0x%04x, PID=0x%04x) matches up with LDD "
                       "<%s>, assigned as handled_device=0x%x",
                       dev->device._device.idVendor,
                       dev->device._device.idProduct, name,
                       dev->assigned_number);
      return;
    }
  }
...
void usb_handler_thread::disconnect_usb_device(std::shared_ptr<usb_device> dev,
  ...
  if (dev->assigned_number && handled_devices.erase(dev->assigned_number)) {
    ...
    sys_usbd.success(
        "USB device(VID=0x%04x, PID=0x%04x) unassigned, handled_device=0x%x",
        dev->device._device.idVendor, dev->device._device.idProduct,
        dev->assigned_number);
    dev->assigned_number = 0;
    std::erase_if(open_pipes,
                  [&](const auto &val) { return val.second.device == dev; });
  }
  if (update_usb_devices) {
    std::erase_if(usb_devices, [&](const auto &val) { return val == dev; });
  }
}
...
void connect_usb_controller(u8 index, input::product_type type) {
  ...
  if (const auto it = usbh->pad_to_usb.find(index);
      it != usbh->pad_to_usb.end()) {
  ...
  if (!already_connected) {
    ...
    case input::product_type::guncon_3: {
      ...
      std::shared_ptr<usb_device> dev =
          std::make_shared<usb_device_guncon3>(index, usbh->get_new_location());
    ...
    case input::product_type::top_shot_elite: {
      if (!g_cfg_topshotelite.load()) {
        sys_usbd.notice(
            "Could not load Top Shot Elite config. Using defaults.");
      }

      sys_usbd.success("Adding emulated Top Shot Elite (controller %d)", index);
      std::shared_ptr<usb_device> dev =
          std::make_shared<usb_device_topshotelite>(index,
    ...
    case input::product_type::top_shot_fearmaster: {
      if (!g_cfg_topshotfearmaster.load()) {
        sys_usbd.notice(
            "Could not load Top Shot Fearmaster config. Using defaults.");
      }

      sys_usbd.success("Adding emulated Top Shot Fearmaster (controller %d)",
                       index);
      std::shared_ptr<usb_device> dev =
          std::make_shared<usb_device_topshotfearmaster>(
    ...
    case input::product_type::udraw_gametablet: {
      sys_usbd.success("Adding emulated uDraw GameTablet (controller %d)",
                       index);
      std::shared_ptr<usb_device> dev = std::make_shared<usb_device_gametablet>(
          index, usbh->get_new_location());
      usbh->connect_usb_device(dev, true);
      usbh->pad_to_usb.emplace(index, std::pair(type, dev));
      break;
...
error_code sys_usbd_get_device_list(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.warning("sys_usbd_get_device_list(handle=0x%x, device_list=*0x%x, "
                   "max_devices=0x%x)",
                   handle, device_list, max_devices);
  ...
  const u32 i_tocopy =
      std::min<u32>(max_devices, ::size32(usbh.handled_devices));
  u32 index = 0;
...
error_code sys_usbd_register_extra_ldd(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.trace(
      "sys_usbd_register_extra_ldd(handle=0x%x, s_product=%s, slen_product=%d, "
      "id_vendor=0x%04x, id_product_min=0x%04x, id_product_max=0x%04x)",
      handle, s_product, slen_product, id_vendor, id_product_min,
      id_product_max);
...
error_code sys_usbd_unregister_extra_ldd(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.trace("sys_usbd_unregister_extra_ldd(handle=0x%x, s_product=%s, "
                 "slen_product=%d)",
                 handle, s_product, slen_product);
...
error_code sys_usbd_get_descriptor_size(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.trace("sys_usbd_get_descriptor_size(handle=0x%x, deviceNumber=0x%x)",
  ...
  return not_an_error(
      usbh.handled_devices[device_handle].second->device.get_size());
}
...
error_code sys_usbd_get_descriptor(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.trace("sys_usbd_get_descriptor(handle=0x%x, deviceNumber=0x%x, "
                 "descriptor=0x%x, desc_size=0x%x)",
                 handle, device_handle, descriptor, desc_size);
  ...
  usbh.handled_devices[device_handle].second->device.write_data(
      reinterpret_cast<u8 *>(descriptor.get_ptr()), desc_size);

  return CELL_OK;
}

// This function is used for psp(cellUsbPspcm), ps3 arcade usj io(PS3A-USJ), ps2
// cam(eyetoy), generic usb camera?(sample_usb2cam)
error_code sys_usbd_register_ldd(ppu_thread &ppu, u32 handle,
  ...
  // The register_ldd appears to be a more promiscuous mode function, where all
  // device 'inserts' would be presented to the cellUsbd for Probing. Unsure how
  // many more devices might need similar treatment (i.e. just a compare and
  // force VID/PID add), or if it's worth adding a full promiscuous capability
  static const std::unordered_map<std::string, UsbLdd, fmt::string_hash,
                                  std::equal_to<>>
      predefined_ldds{{"cellUsbPspcm", {0x054C, 0x01CB, 0x01CB}},
                      {"guncon3", {0x0B9A, 0x0800, 0x0800}},
                      {"PS3A-USJ", {0x0B9A, 0x0900, 0x0910}}};

  if (const auto iterator = predefined_ldds.find(product);
      iterator != predefined_ldds.end()) {
    sys_usbd.trace(
        "sys_usbd_register_ldd(handle=0x%x, s_product=%s, slen_product=%d) -> "
        "Redirecting to sys_usbd_register_extra_ldd()",
        handle, s_product, slen_product);
    return sys_usbd_register_extra_ldd(
        ppu, handle, s_product, slen_product, iterator->second.id_vendor,
        iterator->second.id_product_min, iterator->second.id_product_max);
  }
...
error_code sys_usbd_unregister_ldd(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.trace(
      "sys_usbd_unregister_ldd(handle=0x%x, s_product=%s, slen_product=%d) -> "
      "Redirecting to sys_usbd_unregister_extra_ldd()",
      handle, s_product, slen_product);
...
error_code sys_usbd_open_pipe(ppu_thread &ppu, u32 handle, u32 device_handle,
  ...
  sys_usbd.warning(
      "sys_usbd_open_pipe(handle=0x%x, device_handle=0x%x, unk1=0x%x, "
      "unk2=0x%x, unk3=0x%x, endpoint=0x%x, attributes=0x%x)",
      handle, device_handle, unk1, unk2, unk3, endpoint, attributes);
...
error_code sys_usbd_open_default_pipe(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.trace("sys_usbd_open_default_pipe(handle=0x%x, device_handle=0x%x)",
                 handle, device_handle);
...
error_code sys_usbd_close_pipe(ppu_thread &ppu, u32 handle, u32 pipe_handle) {
  ...
  sys_usbd.todo("sys_usbd_close_pipe(handle=0x%x, pipe_handle=0x%x)", handle,
                pipe_handle);
...
// *arg1 = 3 will do some extra processing right away(notification of transfer
// finishing) *arg1 < 1 || *arg1 > 4 are ignored(rewait instantly for event)
// *arg1 == 1 || *arg1 == 2 will send a sys_event to internal CellUsbd event
// queue with same parameters as received and loop(attach and detach event)
error_code sys_usbd_receive_event(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.trace(
      "sys_usbd_receive_event(handle=0x%x, arg1=*0x%x, arg2=*0x%x, arg3=*0x%x)",
...

  {
    ...
    if (usbh.get_event(arg1, arg2, arg3)) {
      ...
      // Attaching the device too fast seems to result in a nullptr along the
      // way
  ...
  while (auto state = +ppu.state) {
    if (state & cpu_flag::signal &&
      ...
      sys_usbd.trace("Received event(queued): arg1=0x%x arg2=0x%x arg3=0x%x",
                     ppu.gpr[4], ppu.gpr[5], ppu.gpr[6]);
      break;
    }
...
error_code sys_usbd_attach(ppu_thread &ppu, u32 handle, u32 unk1, u32 unk2,
  ...
  sys_usbd.todo(
      "sys_usbd_attach(handle=0x%x, unk1=0x%x, unk2=0x%x, device_handle=0x%x)",
      handle, unk1, unk2, device_handle);
  return CELL_OK;
}
...
error_code sys_usbd_transfer_data(ppu_thread &ppu, u32 handle, u32 id_pipe,
  ...
  sys_usbd.trace("sys_usbd_transfer_data(handle=0x%x, id_pipe=0x%x, buf=*0x%x, "
                 "buf_length=0x%x, request=*0x%x, type=0x%x)",
                 handle, id_pipe, buf, buf_size, request, type_transfer);

  if (sys_usbd.trace && request) {
    sys_usbd.trace("RequestType:0x%02x, Request:0x%02x, wValue:0x%04x, "
    ...
    if ((request->bmRequestType & 0x80) == 0 && buf && buf_size != 0)
      sys_usbd.trace("Control sent:\n%s",
                     fmt::buf_to_hexstring(buf.get_ptr(), buf_size));
  }
  ...
  if (pipe.endpoint == 0) {
    if (!request) {
      sys_usbd.error(
          "Tried to use control pipe without proper request pointer");
      return CELL_EINVAL;
    }

    // Claiming interface
    switch (request->bmRequestType) {
    case 0U /*silences warning*/ | LIBUSB_ENDPOINT_OUT |
    ...
    case 0U /*silences warning*/ | LIBUSB_ENDPOINT_IN |
        LIBUSB_REQUEST_TYPE_STANDARD | LIBUSB_RECIPIENT_DEVICE: {
      if (!buf) {
    ...
    pipe.device->control_transfer(
        request->bmRequestType, request->bRequest, request->wValue,
        request->wIndex, request->wLength, buf_size, buf.get_ptr(), &transfer);
  } else {
    // If output endpoint
    ...
    pipe.device->interrupt_transfer(buf_size, buf.get_ptr(), pipe.endpoint,
                                    &transfer);
  }
...
sys_usbd_isochronous_transfer_data(ppu_thread &ppu, u32 handle, u32 id_pipe,
  ...
  sys_usbd.trace("sys_usbd_isochronous_transfer_data(handle=0x%x, id_pipe=0x%x, "
                 "iso_request=*0x%x)",
                 handle, id_pipe, iso_request);
...
error_code sys_usbd_get_transfer_status(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.trace("sys_usbd_get_transfer_status(handle=0x%x, id_transfer=0x%x, "
                 "unk1=0x%x, result=*0x%x, count=*0x%x)",
                 handle, id_transfer, unk1, result, count);
...
error_code sys_usbd_get_isochronous_transfer_status(
  ...
  sys_usbd.trace("sys_usbd_get_isochronous_transfer_status(handle=0x%x, "
                 "id_transfer=0x%x, unk1=0x%x, request=*0x%x, result=*0x%x)",
                 handle, id_transfer, unk1, request, result);
...
error_code sys_usbd_get_device_location(ppu_thread &ppu, u32 handle,
  ...
  sys_usbd.notice("sys_usbd_get_device_location(handle=0x%x, "
                  "device_handle=0x%x, location=*0x%x)",
                  handle, device_handle, location);
...
error_code sys_usbd_event_port_send(ppu_thread &ppu, u32 handle, u64 arg1,
  ...
  sys_usbd.warning(
      "sys_usbd_event_port_send(handle=0x%x, arg1=0x%x, arg2=0x%x, arg3=0x%x)",
      handle, arg1, arg2, arg3);
