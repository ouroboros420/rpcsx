...
public:
  void add_message(s32 sock_id, const sockaddr_in *dst, std::vector<u8> data,
                   u64 seq) {
    {
    ...
    wakey.release(1);
    wakey.notify_one(); // TODO: Should be improved to only wake if new timeout
                        // < old timeout
  }
  ...
  void confirm_data_received(s32 sock_id, u64 ack) {
    ...
    for (auto it = msgs.begin(); it != msgs.end();) {
      ...
      if (msg.sock_id == sock_id && msg.seq < ack) {
        ...
        if (now < it->first) {
          const auto actual_rtt =
              std::chrono::duration_cast<std::chrono::milliseconds>(
                  now - it->second.initial_sendtime);
    ...
    while (thread_ctrl::state() != thread_state::aborting) {
      ...
      for (auto it = msgs.begin(); it != msgs.end();) {
        ...
        if (rtt.num_retries >= 10) {
          // Too many retries, need to notify the socket that the connection is
          // dead
        ...
        // resend the message
        const auto res = idm::check<
            lv2_socket>(msg.sock_id, [&](lv2_socket &sock) -> bool {
          ensure(sock.get_type() == SYS_NET_SOCK_STREAM_P2P);
          auto &sock_p2ps = reinterpret_cast<lv2_socket_p2ps &>(sock);

          while (np::sendto_possibly_ipv6(
                     sock_p2ps.get_socket(),
                     reinterpret_cast<const char *>(msg.data.data()),
                     ::size32(msg.data), &msg.dst_addr, 0) == -1) {
            const sys_net_error err = get_last_error(false);
            // concurrency on the socket(from a sendto for example) can result
            // in EAGAIN error in which case we try again
            if (err == SYS_NET_EAGAIN) {
              continue;
            }

            sys_net.error(
                "[P2PS] Resending the packet failed(%s), closing the stream",
                err);
            sock_p2ps.close_stream();
            return false;
          }
      ...
      if (!msgs.empty()) {
        ...
        if (current_timepoint > expected_timepoint) {
          ...
          timeout = static_cast<atomic_wait_timeout>(
              std::chrono::duration_cast<std::chrono::nanoseconds>(
                  expected_timepoint - current_timepoint)
                  .count());
        }
      } else {
...
std::vector<u8> generate_u2s_packet(const p2ps_encapsulated_tcp &header,
  ...
  const u32 packet_size =
      (VPORT_P2P_HEADER_SIZE + sizeof(p2ps_encapsulated_tcp) + datasize);
  ensure(packet_size <
         65535); // packet size shouldn't be bigger than possible UDP payload
  ...
  memcpy(packet_data + VPORT_P2P_HEADER_SIZE, &header,
         sizeof(p2ps_encapsulated_tcp));
  if (datasize)
    memcpy(packet_data + VPORT_P2P_HEADER_SIZE + sizeof(p2ps_encapsulated_tcp),
           data, datasize);

  auto *hdr_ptr = reinterpret_cast<p2ps_encapsulated_tcp *>(
      packet_data + VPORT_P2P_HEADER_SIZE);
  hdr_ptr->checksum = 0;
  hdr_ptr->checksum =
      u2s_tcp_checksum(utils::bless<le_t<u16>>(hdr_ptr),
                       sizeof(p2ps_encapsulated_tcp) + datasize);
...
lv2_socket_p2ps::lv2_socket_p2ps(socket_type native_socket, u16 port, u16 vport,
                                 u32 op_addr, u16 op_port, u16 op_vport,
                                 u64 cur_seq, u64 data_beg_seq, s32 so_nbio)
    : lv2_socket_p2p(SYS_NET_AF_INET, SYS_NET_SOCK_STREAM_P2P,
                     SYS_NET_IPPROTO_IP) {
...
lv2_socket_p2ps::lv2_socket_p2ps(utils::serial &ar, lv2_socket_type type)
  ...
  ar(status, max_backlog, backlog, op_port, op_vport, op_addr, data_beg_seq,
     received_data, cur_seq);
}
...
bool lv2_socket_p2ps::handle_connected(p2ps_encapsulated_tcp *tcp_header,
                                       u8 *data, ::sockaddr_storage *op_addr,
                                       nt_p2p_port *p2p_port) {
  std::lock_guard lock(mutex);

  if (status != p2ps_stream_status::stream_connected &&
      status != p2ps_stream_status::stream_handshaking) {
    sys_net.error("[P2PS] lv2_socket_p2ps::handle_connected() called on a non "
                  "connected/handshaking socket(%d)!",
  ...
  auto send_ack = [&]() {
    ...
    send_u2s_packet(std::move(packet),
                    reinterpret_cast<::sockaddr_in *>(op_addr), 0, false);
  ...
  if (status == p2ps_stream_status::stream_handshaking) {
    ...
    if (tcp_header->flags == (p2ps_tcp_flags::SYN | p2ps_tcp_flags::ACK)) {
      ...
      sys_net.error("[P2PS] Unexpected U2S TCP flag received with handshaking "
                    "state: 0x%02X",
                    tcp_header->flags);
    }
    ...
    case p2ps_tcp_flags::FIN: {
      sys_net.error("[P2PS] Received RST/FIN packet(%d), closing the stream",
                    tcp_header->flags);
      close_stream_nl(p2p_port);
      return false;
    }
    default: {
      sys_net.error("[P2PS] Unexpected U2S TCP flag received with connected "
                    "state: 0x%02X",
                    tcp_header->flags);
      return true;
    }
...
bool lv2_socket_p2ps::handle_listening(p2ps_encapsulated_tcp *tcp_header,
  ...
  if (status != p2ps_stream_status::stream_listening) {
    sys_net.error("[P2PS] lv2_socket_p2ps::handle_listening() called on a non "
                  "listening socket(%d)!",
                  static_cast<u8>(status));
    return false;
  }
  ...
  if (tcp_header->flags == static_cast<u8>(p2ps_tcp_flags::SYN)) {
    ...
    // Yes, new connection and a backlog is available, create a new lv2_socket
    // for it and send SYN|ACK Prepare reply packet
    sys_net.notice(
        "[P2PS] Received connection on listening STREAM-P2P socket!");
    p2ps_encapsulated_tcp send_hdr;
    send_hdr.src_port = tcp_header->dst_port;
    send_hdr.dst_port = tcp_header->src_port;
    ...
    // Create new socket
    const u32 new_op_addr =
        reinterpret_cast<struct sockaddr_in *>(op_addr)->sin_addr.s_addr;
    const u16 new_op_port = std::bit_cast<u16, be_t<u16>>(
        (reinterpret_cast<struct sockaddr_in *>(op_addr)->sin_port));
    const u16 new_op_vport = tcp_header->src_port;
    const u64 new_cur_seq = send_hdr.seq + 1;
    const u64 new_data_beg_seq = send_hdr.ack;
    auto sock_lv2 = make_shared<lv2_socket_p2ps>(
        native_socket, port, vport, new_op_addr, new_op_port, new_op_vport,
        new_cur_seq, new_data_beg_seq, so_nbio);
    ...
    auto packet = generate_u2s_packet(send_hdr, nullptr, 0);
    {
      ...
      sock_lv2->send_u2s_packet(std::move(packet),
                                reinterpret_cast<::sockaddr_in *>(op_addr),
                                send_hdr.seq, true);
    }
    ...
    sys_net.error(
        "[P2PS] Unexpected U2S TCP flag received on listening socket: 0x%02X",
        tcp_header->flags);
  }
...
void lv2_socket_p2ps::send_u2s_packet(std::vector<u8> data,
                                      const ::sockaddr_in *dst, u64 seq,
                                      bool require_ack) {
  char ip_str[16];
  inet_ntop(AF_INET, &dst->sin_addr, ip_str, sizeof(ip_str));
  sys_net.trace("[P2PS] Sending U2S packet on socket %d(id:%d): data(%d, seq "
                "%d, require_ack %d) to %s:%d",
                native_socket, lv2_id, data.size(), seq, require_ack, ip_str,
                std::bit_cast<u16, be_t<u16>>(dst->sin_port));

  while (np::sendto_possibly_ipv6(native_socket,
                                  reinterpret_cast<char *>(data.data()),
                                  ::size32(data), dst, 0) == -1) {
    const sys_net_error err = get_last_error(false);
    // concurrency on the socket can result in EAGAIN error in which case we try
    // again
    if (err == SYS_NET_EAGAIN) {
      continue;
    }
...
void lv2_socket_p2ps::close_stream_nl(nt_p2p_port *p2p_port) {
  ...
  for (auto it = p2p_port->bound_p2p_streams.begin();
       it != p2p_port->bound_p2p_streams.end();) {
    if (it->second == lv2_id) {
...
p2ps_stream_status lv2_socket_p2ps::get_status() const { return status; }
...
std::pair<s32, sys_net_sockaddr> lv2_socket_p2ps::getpeername() {
  ...
  sys_net_sockaddr_in_p2p *p2p_addr =
      reinterpret_cast<sys_net_sockaddr_in_p2p *>(&res);
...
lv2_socket_p2ps::accept(bool is_lock) {
...
s32 lv2_socket_p2ps::bind(const sys_net_sockaddr &addr) {
  ...
  // For SYS_NET_SOCK_STREAM_P2P sockets, the port is the "fake" tcp port and
  // the vport is the udp port it's bound to
  u16 p2p_port = psa_in_p2p->sin_vport;
  u16 p2p_vport = psa_in_p2p->sin_port;

  sys_net.notice("[P2PS] Trying to bind %s:%d:%d",
  ...
  if (p2p_port != SCE_NP_PORT) {
    sys_net.warning("[P2PS] Attempting to bind a socket to a port != %d",
                    +SCE_NP_PORT);
  }
  ...
  auto &nc = g_fxo->get<p2p_context>();
  {
    ...
    {
      // Ensures the socket & the bound list are updated at the same time to
      // avoid races
      ...
      if (p2p_vport == 0) {
        ...
        pport.bound_p2ps_vports.insert(
...
std::optional<s32> lv2_socket_p2ps::connect(const sys_net_sockaddr &addr) {
  ...
  auto &nc = g_fxo->get<p2p_context>();
  {
...

    {
      ...
      if (vport == 0) {
        ...
        while (pport.bound_p2p_vports.count(vport) ||
               pport.bound_p2p_streams.count(static_cast<u64>(vport) << 32)) {
          vport = pport.get_port();
        }
      }
      const u64 key = name.sin_addr.s_addr | (static_cast<u64>(vport) << 32) |
                      (static_cast<u64>(dst_vport) << 48);
  ...
  send_u2s_packet(std::move(packet), reinterpret_cast<::sockaddr_in *>(&name),
                  send_hdr.seq, true);
...
lv2_socket_p2ps::recvfrom([[maybe_unused]] s32 flags, u32 len, bool is_lock) {
  std::unique_lock<shared_mutex> lock(mutex, std::defer_lock);
  ...
  sys_net.trace("[P2PS] STREAM-P2P socket had %u available, given %u",
                data_available, to_give);

  u32 left_to_give = to_give;
  while (left_to_give) {
    auto &cur_data = received_data.begin()->second;
    auto to_give_for_this_packet =
        std::min(static_cast<u32>(cur_data.size()), left_to_give);
    memcpy(dest_buf.data() + (to_give - left_to_give), cur_data.data(),
    ...
    if (cur_data.size() != to_give_for_this_packet) {
      ...
      memcpy(new_vec.data(), cur_data.data() + to_give_for_this_packet,
             amount_left);
  ...
  sys_net_sockaddr_in_p2p *addr_p2p =
      reinterpret_cast<sys_net_sockaddr_in_p2p *>(&addr);
...
std::optional<s32>
lv2_socket_p2ps::sendto([[maybe_unused]] s32 flags, const std::vector<u8> &buf,
  ...
  constexpr u32 max_data_len =
      (65535 - (VPORT_P2P_HEADER_SIZE + sizeof(p2ps_encapsulated_tcp)));
  ...
  while (cur_total_len > 0) {
    ...
    auto packet = generate_u2s_packet(
        tcp_header, &buf[buf.size() - cur_total_len], cur_data_len);
...
std::optional<s32>
lv2_socket_p2ps::sendmsg([[maybe_unused]] s32 flags,
